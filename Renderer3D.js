...
const Renderer3D = (() => {
    ...
    let snakeMesh = null;
    let snakeCurvePoints = []; // Reusable array for snake curve calculation
    ...
    let dummyObject = new THREE.Object3D(); // For instanced mesh updates
    ...
    const SPARK_BASE_LIFE = 0.18; const SPARK_RAND_LIFE = 0.15; // Adjusted spark life
    ...
    function init(containerElement, initialWidth, initialHeight) {
        ...
        cameraTargetPos.set(gameWidth / 2, 0, gameHeight / 2); // Initialize camera target
        try {
            ...
            containerElement.appendChild(renderer.domElement); // Append to the passed container
            ...
            directionalLight.shadow.camera.left = -gameWidth; // Adjust shadow bounds for larger map
            ...
            scene.add(directionalLight.target); // Important for light direction
            ...
            groundPlane.scale.set(gameWidth * 1.1, gameHeight * 1.1, 1); // Use potentially larger dimensions
    ...
    function handleResize() {
        ...
        camera.position.set(cameraTargetPos.x, 1000, cameraTargetPos.z); // Reset position based on target
        camera.lookAt(cameraTargetPos.x, 0, cameraTargetPos.z); // Look at target
    ...
    function createPowerupMesh(powerupData){
        ...
        const iconMesh = new THREE.Mesh(iconGeo, mat); iconMesh.castShadow = true; iconMesh.receiveShadow = true; iconMesh.position.y = POWERUP_Y_OFFSET; iconMesh.rotation.set(Math.PI / 6, 0, Math.PI / 6); // Initial rotation
    ...
    function updateMeshes(state, meshDict, createFn, defaultYPosFn, localEffects, appState) {
        ...
        if (state) {
            for (const id in state) {
                ...
                if (bodyMesh && bodyMesh.material && !isNew) { // Apply effects only after creation frame
                    ...
                    if (meshDict === enemyMeshes && data.health <= 0 && data.death_timestamp) {
                        ...
                        obj.visible = isVisible; // Hide whole group when faded
                    ...
                    if(isPushback) {
                        ...
                        emissiveIntensity = Math.sin(pushbackProgress*Math.PI)*0.8;
                        emissiveColor = 0x66ccff;
                    } else if (isBitten) {
                        emissiveIntensity = 0.4 + Math.sin(now * 0.005) * 0.3;
                        emissiveColor = 0x00ff00;
                    }
                    if(bodyMesh.material.emissive) {
                        bodyMesh.material.emissive.setHex(emissiveColor);
                        bodyMesh.material.emissiveIntensity = emissiveIntensity;
                    }
                }

                if (iconMesh) { // Powerup animation
                    iconMesh.rotation.y += 0.02;
                    iconMesh.position.y = POWERUP_Y_OFFSET + Math.sin(performance.now() * 0.002) * 3;
                }

                if (meshDict === playerMeshes && obj.userData?.gunMesh) { // Player aiming
                     const aimDx = data.id === appState?.localPlayerId ? appState.localPlayerAimState.lastAimDx : 0;
                     const aimDy = data.id === appState?.localPlayerId ? appState.localPlayerAimState.lastAimDy : -1;
                     obj.rotation.y = Math.atan2(aimDx, aimDy); // Orient whole group
                } else if (meshDict === enemyMeshes && obj.userData?.type === 'shooter') {
                     // Basic enemy aiming (optional)
                     const targetPlayer = appState?.serverState?.players?.[data.target_player_id];
                     if (targetPlayer) {
                         const aimTargetX = targetPlayer.x;
                         const aimTargetZ = targetPlayer.y;
                         obj.rotation.y = Math.atan2(aimTargetX - data.x, aimTargetZ - data.y);
                     }
                }
            }
        }
    ...
    function disposeObject3D(obj) {
        ...
        if (obj instanceof THREE.Mesh || obj instanceof THREE.Points || obj instanceof THREE.LineSegments) {
            ...
            if (obj.material) { if (Array.isArray(obj.material)) obj.material.forEach(m => m?.dispose()); else if (obj.material.dispose) obj.material.dispose(); }
    ...
    function getYOffsetForEntity(entityData) {
         if (entityData.radius) return BULLET_Y_OFFSET; // Bullets have radius
         return 0; // Groups positioned at ground level
    }

    function updateEnvironment(isNight) {
        const dayLightIntensity=1.0, nightLightIntensity=0.45; const dayAmbientIntensity=0.6, nightAmbientIntensity=0.3; const dayDirColor=0xffffff, nightDirColor=0xc0d0ff; const dayAmbColor=0xffffff, nightAmbColor=0x556699;
        ...
        scene.fog = isNight ? new THREE.FogExp2(0x081020, 0.001) : null;
    }

    function updateMuzzleFlash(appState, flashState) {
        if (!muzzleFlashLight || !appState?.localPlayerId) return;
        const playerGroup = playerMeshes[appState.localPlayerId];
        if (!playerGroup || !playerGroup.userData.gunMesh) return;
        const now = performance.now();
        if (flashState.active && now < flashState.endTime) {
            muzzleFlashLight.intensity = 4.0 + Math.random() * 3.0;
            const offsetDistance = DEF_GUN_LENGTH * 0.7;
            const gunMesh = playerGroup.userData.gunMesh;
            const worldPos = new THREE.Vector3();
            gunMesh.getWorldPosition(worldPos); // Get gun world position
            // Simplified offset based on player group's rotation (aim direction)
            const aimDirection = new THREE.Vector3(appState.localPlayerAimState.lastAimDx, 0, appState.localPlayerAimState.lastAimDy);
            worldPos.addScaledVector(aimDirection, offsetDistance);
    ...
    function updateHitSparks(activeSparkEffects, deltaTime) {
        ...
        for (const enemyId in activeSparkEffects) {
            ...
            if (now < effectEndTime && enemyMesh) {
                const sparksToSpawn = 4 + Math.floor(Math.random() * 5);
                const enemyPos = enemyMesh.position;
                const enemyYOffset = (enemyMesh.userData?.type === 'giant' ? GIANT_Y_OFFSET : ENEMY_Y_OFFSET);
                for (let i = 0; i < sparksToSpawn; i++) {
                    let foundSlot = false;
                    for (let j = firstInactiveIndex; j < MAX_SPARKS; j++) {
                        const p = sparkData[j];
                        if (p.life <= 0) {
                            p.position.copy(enemyPos);
                            p.position.y = enemyYOffset * (0.2 + Math.random() * 0.8);
                            const angle = Math.random() * Math.PI * 2;
                            const speed = 120 + Math.random() * 90;
                            p.velocity.set(Math.cos(angle)*speed, (Math.random()-0.3)*speed*0.6, Math.sin(angle)*speed);
                            p.color.setRGB(1, Math.random()*0.4, 0);
                            p.alpha = 1.0;
                            p.life = SPARK_BASE_LIFE + Math.random() * SPARK_RAND_LIFE;
                            firstInactiveIndex = j + 1;
                            foundSlot = true;
                            break;
                        }
                    }
                ...
                delete activeSparkEffects[enemyId]; // Consume the effect trigger
            } else if (now >= effectEndTime) {
                 delete activeSparkEffects[enemyId]; // Clean up expired trigger
            }
        }
        ...
        for (let i = 0; i < MAX_SPARKS; i++) {
            ...
            if (p.life > 0) {
                ...
                if (p.life <= 0) {
                    p.alpha = 0.0;
                    alphas[i] = 0.0;
                    positions[i * 3 + 1] = -1000; // Move definitively off-screen
                } else {
                    p.position.addScaledVector(p.velocity, deltaTime);
                    p.velocity.y -= 250 * deltaTime; // Gravity
                    p.alpha = Math.max(0, (p.life / (SPARK_BASE_LIFE + SPARK_RAND_LIFE)) * 1.2); // Fade out
                    alphas[i] = p.alpha;
                ...
                needsUpdate = true; // Mark for update if any particle was active
            } else if (alphas[i] > 0) {
                 alphas[i] = 0; // Ensure inactive particle alpha is zeroed in buffer
                 positions[i * 3 + 1] = -1000; // Ensure position is off-screen
                 needsUpdate = true; // Mark for update if we had to zero out alpha
            }
        }
        ...
        hitSparkParticles.visible = needsUpdate; // Only show if particles might be visible
    }
    ...
    function updateAmmoCasings(activeCasings) {
        ...
        for(let i=0; i<activeCasings.length && i<MAX_CASINGS; i++){
            ...
            dummyObject.position.set(casing.x, DEF_CASING_LENGTH/2 + 0.5, casing.y); // Position on ground
            dummyObject.rotation.set(Math.PI/2 + (Math.random()-0.5)*0.4, casing.rotation, (Math.random()-0.5)*0.4); // Random tumble
            dummyObject.updateMatrix();
            ammoCasingMesh.setMatrixAt(i, dummyObject.matrix);
            visibleCount++;
        }
    ...
    function updateWeatherParticles(appState, deltaTime) {
        if (!appState) return;
        if (rainLines && rainGeometry) {
            rainLines.visible = appState.isRaining;
            if (appState.isRaining) {
                const positions = rainGeometry.attributes.position.array;
                const rainSpeed = 450;
                const streakLengthFactor = 0.04;
                for (let i = 0; i < MAX_RAIN_DROPS; i++) {
                    const p = rainData[i];
                    p.position.y -= rainSpeed * deltaTime;
                    if (p.position.y < -20) {
                        p.position.x = Math.random()*gameWidth*1.2 - gameWidth*0.1;
                        p.position.y = gameHeight*1.2 + Math.random() * 100;
                        p.position.z = Math.random()*gameHeight*1.2 - gameHeight*0.1;
                    }
                    const endY = p.position.y - rainSpeed * streakLengthFactor;
                    const idx = i * 6;
                    positions[idx + 0] = p.position.x; positions[idx + 1] = p.position.y; positions[idx + 2] = p.position.z;
                    positions[idx + 3] = p.position.x; positions[idx + 4] = endY; positions[idx + 5] = p.position.z;
                }
        ...
        if (dustParticles && dustGeometry) {
            ...
            if (appState.isDustStorm) {
                ...
                for (let i = 0; i < MAX_DUST_MOTES; i++) {
                    const p = dustData[i];
                    p.position.addScaledVector(p.velocity, deltaTime);
                    p.velocity.x += (Math.random()-0.5)*50*deltaTime;
    ...
    function updateSnake(snakeData) {
        ...
        if(snakeData.isActiveFromServer && snakeData.segments && snakeData.segments.length > 1){
             ...
             if (snakeCurvePoints.length >= 2) {
                 ...
                 const frames = curve.computeFrenetFrames(DEF_SNAKE_SEGMENTS, false); // Use built-in Frenet frames

                 let P = new THREE.Vector3(); let N = new THREE.Vector3(); let B = new THREE.Vector3();
                 let vertexIndex = 0;
                 ...
                 for ( let i = 0; i <= segments; i ++ ) {
                     P = curve.getPointAt(i / segments, P); // Get point on curve
                     ...
                     for ( let j = 0; j <= radiusSegments; j ++ ) {
                         ...
                         let normal = new THREE.Vector3(); // Create normal vector
                         normal.x = ( cos * N.x + sin * B.x ); normal.y = ( cos * N.y + sin * B.y ); normal.z = ( cos * N.z + sin * B.z );
                         normal.normalize();
                         let position = new THREE.Vector3(); // Create position vector
                         position.copy(P).addScaledVector(normal, DEF_SNAKE_RADIUS); // Add radius offset
                         positions[vertexIndex++] = position.x; positions[vertexIndex++] = position.y; positions[vertexIndex++] = position.z;
                     }
                 }
                 tubeGeometry.attributes.position.needsUpdate = true;
                 tubeGeometry.computeVertexNormals(); // Update normals for lighting
             } else {
                  snakeMesh.visible = false;
             }
        } else {
        ...
        if (campfireData.active) {
            ...
            if (flameParticles && flameGeometry) {
                 ...
                 for(let i=0; i < MAX_FLAMES; i++) {
                     const p = flameData[i];
                     if (p.life <= 0 && Math.random() < 0.2 && i >= firstInactiveFlame) {
                         p.position.set((Math.random()-0.5)*15, FLAME_PARTICLE_Y_OFFSET, (Math.random()-0.5)*15); p.velocity.set((Math.random()-0.5)*18, 50 + Math.random()*30, (Math.random()-0.5)*18); p.life = p.baseLife; p.alpha = 0.8 + Math.random()*0.2; p.color.setHSL(0.08 + Math.random()*0.04, 1.0, 0.6 + Math.random()*0.1); firstInactiveFlame = i + 1;
                     }
                     if (p.life > 0) {
                          p.life -= deltaTime;
    ...
    function getScreenPosition(worldPosition, cameraRef, rendererRef) {
        try {
            ...
            const widthHalf = rendererRef.domElement.width / 2;
            const heightHalf = rendererRef.domElement.height / 2;
            const screenX = Math.round((vector.x * widthHalf) + widthHalf);
            const screenY = Math.round(-(vector.y * heightHalf) + heightHalf);
            return{ x: screenX, y: screenY };
        } catch (e) {
            console.error("Error getScreenPosition:", e, "WorldPos:", worldPosition);
            return { x: 0, y: 0 };
        }
    }

    function renderScene(stateToRender, appState, localEffects, deltaTime = 0.016) { // Add deltaTime default
        ...
        if (appState.canvasWidth && appState.canvasHeight && (gameWidth !== appState.canvasWidth || gameHeight !== appState.canvasHeight)) {
             ...
             cameraTargetPos.x = gameWidth / 2; cameraTargetPos.z = gameHeight / 2; // Re-center target if map size changes
        }
        ...
        if (appState.localPlayerId && playerMeshes[appState.localPlayerId]) {
            cameraTargetPos.x = lerp(cameraTargetPos.x, appState.renderedPlayerPos.x, cameraLerpFactor);
            cameraTargetPos.z = lerp(cameraTargetPos.z, appState.renderedPlayerPos.y, cameraLerpFactor); // appState y is world Z
        }
        ...
        const finalCamX = cameraTargetPos.x + screenShakeOffset.x;
        const finalCamZ = cameraTargetPos.z + screenShakeOffset.z;
        camera.position.set(finalCamX, 1000, finalCamZ);
        camera.lookAt(finalCamX, 0, finalCamZ);
        ...
        updateMeshes(stateToRender.players, playerMeshes, createPlayerMesh, (d) => PLAYER_Y_OFFSET, localEffects, appState);
        updateMeshes(stateToRender.enemies, enemyMeshes, createEnemyMesh, (d) => (d.type === 'giant' ? GIANT_Y_OFFSET : ENEMY_Y_OFFSET), localEffects, appState);
        updateMeshes(stateToRender.bullets, bulletMeshes, createBulletMesh, getYOffsetForEntity, localEffects, appState);
        updateMeshes(stateToRender.powerups, powerupMeshes, createPowerupMesh, (d) => 0, localEffects, appState);

        const uiPositions = {};
        const calculateUIPos = (meshDict, stateDict, yOffsetValFn) => {
            for (const id in meshDict) {
                const obj = meshDict[id];
                const data = stateDict?.[id];
                if (obj && data && obj.visible !== false) {
                    ...
                    // Use calculated Y offset for accurate positioning on top of model
                    worldPos.y = yOffsetValFn(data, actualMesh);
                    const screenPos = getScreenPosition(worldPos, camera, renderer);
                    uiPositions[id] = { screenX: screenPos.x, screenY: screenPos.y };
                }
            }
        };
        const getPlayerHeadY = (d,m) => PLAYER_BODY_Y_OFFSET + DEF_PLAYER_HEAD_RADIUS*1.5;
        const getEnemyHeadY = (d,m) => (d.type === 'giant' ? GIANT_Y_OFFSET : ENEMY_Y_OFFSET) + (d.type === 'giant' ? DEF_ENEMY_HEAD_RADIUS * DEF_GIANT_MULTIPLIER : DEF_ENEMY_HEAD_RADIUS)*1.5;
        const getPowerupTopY = (d,m) => POWERUP_Y_OFFSET + DEF_POWERUP_SIZE * 0.5; // Approx top of powerup
        calculateUIPos(playerMeshes, stateToRender.players, getPlayerHeadY);
        calculateUIPos(enemyMeshes, stateToRender.enemies, getEnemyHeadY);
        calculateUIPos(powerupMeshes, stateToRender.powerups, getPowerupTopY);
        if (stateToRender.damage_texts) { // Calculate screen pos for damage text based on world coords
            for (const id in stateToRender.damage_texts) {
                const dt = stateToRender.damage_texts[id];
                const worldPos = new THREE.Vector3(dt.x, PLAYER_BODY_Y_OFFSET, dt.y); // Assume damage text starts near player body height
                const screenPos = getScreenPosition(worldPos, camera, renderer);
                uiPositions[id] = { screenX: screenPos.x, screenY: screenPos.y };
            }
        }
        ...
        try {
            ...
            if (appState.animationFrameId) { cancelAnimationFrame(appState.animationFrameId); appState.animationFrameId = null; console.error("Stopped loop due to render error.");}
        }
    }

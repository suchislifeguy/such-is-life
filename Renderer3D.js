// Renderer3D.js v3.4
// Focus: Fixing initialization ReferenceError.
// Reverted to dynamic world scaling as per previous step for debugging main loop.

import * as THREE from 'three';

// --- Constants ---
const DEFAULT_GAME_WIDTH = 1600;
...
const GROUND_MARGIN = 1.2; // Using dynamic scaling for now
...
let ambientLight, directionalLight, muzzleFlashLight; // Declare muzzleFlashLight here
let domContainer;
...
// Effects
// let muzzleFlashLight = null; // Moved declaration to top
...
function _updateRain(deltaTime) {
    if (!rainSystem || !rainSystem.lines.visible) { return; } const positions = rainSystem.geometry.attributes.position.array; const data = rainSystem.data; let needsUpdate = false; const currentWorldWidth = currentCanvasWidth * GROUND_MARGIN; const currentWorldHeight = currentCanvasHeight * GROUND_MARGIN; const worldOriginX = currentCanvasWidth / 2 - currentWorldWidth / 2; const worldOriginZ = currentCanvasHeight / 2 - currentWorldHeight / 2; for (let i = 0; i < MAX_RAIN_DROPS; i++) { const p = data[i]; p.y += p.s * deltaTime; if (p.y < -50) { p.x = Math.random() * currentWorldWidth + worldOriginX; p.y = Math.random() * 500 + 1000; p.z = Math.random() * currentWorldHeight + worldOriginZ; p.s = RAIN_SPEED_Y + Math.random() * RAIN_SPEED_Y_RAND; } const idx = i * 6; positions[idx + 0] = p.x; positions[idx + 1] = p.y; positions[idx + 2] = p.z; positions[idx + 3] = p.x; positions[idx + 4] = p.y - RAIN_STREAK_LENGTH; positions[idx + 5] = p.z; needsUpdate = true; } if (needsUpdate) rainSystem.geometry.attributes.position.needsUpdate = true;
}

function _updateDust(deltaTime) {
     if (!dustSystem || !dustSystem.particles.visible || !camera) { return; } const positions = dustSystem.geometry.attributes.position.array; const data = dustSystem.data; let needsUpdate = false; const currentWorldWidth = currentCanvasWidth * GROUND_MARGIN; const currentWorldHeight = currentCanvasHeight * GROUND_MARGIN; const worldOriginX = currentCanvasWidth / 2 - currentWorldWidth / 2; const worldOriginZ = currentCanvasHeight / 2 - currentWorldHeight / 2; for (let i = 0; i < MAX_DUST_MOTES; i++) { const p = data[i]; p.p.addScaledVector(p.v, deltaTime); if (p.p.x < worldOriginX) p.p.x += currentWorldWidth; else if (p.p.x > worldOriginX + currentWorldWidth) p.p.x -= currentWorldWidth; if (p.p.z < worldOriginZ) p.p.z += currentWorldHeight; else if (p.p.z > worldOriginZ + currentWorldHeight) p.p.z -= currentWorldHeight; p.p.y += (Math.random() - 0.5) * DUST_SPEED_Y * deltaTime; p.p.y = Math.max(5, Math.min(80, p.p.y)); positions[i * 3 + 0] = p.p.x; positions[i * 3 + 1] = p.p.y; positions[i * 3 + 2] = p.p.z; needsUpdate = true; } if (needsUpdate) dustSystem.geometry.attributes.position.needsUpdate = true;
}
...
function _updateCampfire(deltaTime) {
     if (!campfireSystem || !clock) return; const cfData = window.appState?.serverState?.campfire; const isActive = cfData?.active ?? false; campfireSystem.group.visible = isActive; if (isActive) { campfireSystem.group.position.set(cfData.x, 0, cfData.y); if (campfireSystem.glowLight) { campfireSystem.glowLight.intensity = 2.5 + Math.sin(clock.elapsedTime * 3.0 + Math.random()*0.5) * 0.8; } _updateCampfireFlames(deltaTime); _updateCampfireSmoke(deltaTime); } else { campfireSystem.flameParticles.visible = false; campfireSystem.smokeParticles.visible = false; if (campfireSystem.glowLight) campfireSystem.glowLight.intensity = 0; }
}
...
const Renderer3D = {
    init: (containerElement /* No world size params needed now */ ) => {
        ...
        try {
            ...
            // Shadow bounds updated in resize
            directionalLight.target.position.set(initialTargetX, 0, initialTargetZ);
            scene.add(directionalLight); scene.add(directionalLight.target);

            // *** FIX: Create muzzleFlashLight *before* adding it ***
            muzzleFlashLight = new THREE.PointLight(0xffcc66, 0, 150, 1.8); // Create the light
            muzzleFlashLight.castShadow = false; // Set properties
            scene.add(muzzleFlashLight); // THEN add to scene
            ...
            scene.add(groundPlane); // Scaled in resize

            _createBoundaries(); // Created here, positioned/scaled in resize

            // *** FIX: Initialize systems AFTER basic scene/lights are ready ***
            ...
            Renderer3D.handleContainerResize(); // Initial resize
            setTimeout(() => Renderer3D.handleContainerResize(), 150); // Debounce resize

        } catch (error) {
            console.error("Renderer Init Error:", error); // Log the specific error
            Renderer3D.cleanup(); // Attempt cleanup
            return false;
        }
    ...
    handleContainerResize: () => {
        ...
        const worldCenterX = currentCanvasWidth / 2; const worldCenterZ = currentCanvasHeight / 2; const effectiveWorldWidth = currentCanvasWidth * GROUND_MARGIN; const effectiveWorldHeight = currentCanvasHeight * GROUND_MARGIN;
        ...
        if (groundPlane) { groundPlane.scale.set(effectiveWorldWidth, effectiveWorldHeight, 1); groundPlane.position.set(worldCenterX, 0, worldCenterZ); }
        if (directionalLight?.target) { directionalLight.target.position.set(worldCenterX, 0, worldCenterZ); }
        if (directionalLight?.shadow?.camera) { const sX = effectiveWorldWidth * 0.55; const sZ = effectiveWorldHeight * 0.55; directionalLight.shadow.camera.left = -sX; directionalLight.shadow.camera.right = sX; directionalLight.shadow.camera.top = sZ; directionalLight.shadow.camera.bottom = -sZ; directionalLight.shadow.camera.updateProjectionMatrix(); }
        if (boundariesGroup) { const wallTop = boundariesGroup.getObjectByName("WallTop"); const wallBottom = boundariesGroup.getObjectByName("WallBottom"); const wallLeft = boundariesGroup.getObjectByName("WallLeft"); const wallRight = boundariesGroup.getObjectByName("WallRight"); if(wallTop) { wallTop.scale.x = effectiveWorldWidth + BOUNDARY_WALL_DEPTH * 2; wallTop.position.set(worldCenterX, Y_OFFSET_BOUNDARY, worldCenterZ + effectiveWorldHeight / 2 + BOUNDARY_WALL_DEPTH / 2); } if(wallBottom) { wallBottom.scale.x = effectiveWorldWidth + BOUNDARY_WALL_DEPTH * 2; wallBottom.position.set(worldCenterX, Y_OFFSET_BOUNDARY, worldCenterZ - effectiveWorldHeight / 2 - BOUNDARY_WALL_DEPTH / 2); } if(wallLeft) { wallLeft.scale.x = effectiveWorldHeight; wallLeft.position.set(worldCenterX - effectiveWorldWidth / 2 - BOUNDARY_WALL_DEPTH / 2, Y_OFFSET_BOUNDARY, worldCenterZ); } if(wallRight) { wallRight.scale.x = effectiveWorldHeight; wallRight.position.set(worldCenterX + effectiveWorldWidth / 2 + BOUNDARY_WALL_DEPTH / 2, Y_OFFSET_BOUNDARY, worldCenterZ); } }
    ...
    renderScene: (stateToRender, appState, localEffects) => {
        ...
        const deltaTime = clock.getDelta(); const localPlayerGroup = appState.localPlayerId ? playerGroupMap[appState.localPlayerId] : null;
        _updateCamera(deltaTime, localPlayerGroup); _updateEnvironment(stateToRender.is_night, stateToRender.is_raining, stateToRender.is_dust_storm);
        _syncSceneObjects(stateToRender.players, playerGroupMap, _createPlayerGroup, _updatePlayerGroup, (id) => id === appState.localPlayerId); _syncSceneObjects(stateToRender.enemies, enemyGroupMap, _createEnemyGroup, _updateEnemyGroup); _syncSceneObjects(stateToRender.powerups, powerupGroupMap, _createPowerupGroup, _updatePowerupGroup);
        _updateInstancedMesh(playerBulletMesh, playerBulletMatrices, stateToRender.bullets, Y_OFFSET_BULLET, true); _updateInstancedMesh(enemyBulletMesh, enemyBulletMatrices, stateToRender.bullets, Y_OFFSET_BULLET, true);
    ...
    cleanup: () => {
        ...
        // Use try/catch blocks for each cleanup step to make it more robust
        try { [hitSparkSystem, rainSystem, dustSystem, campfireSystem].forEach(system => { if (system) { if (system.particles) scene?.remove(system.particles); if (system.lines) scene?.remove(system.lines); if (system.group) scene?.remove(system.group); system.geometry?.dispose(); if(system.flameGeometry) system.flameGeometry.dispose(); if(system.smokeGeometry) system.smokeGeometry.dispose(); _disposeMaterialTextures(system.material); system.material?.dispose(); if(system.flameMaterial){_disposeMaterialTextures(system.flameMaterial); system.flameMaterial.dispose();} if(system.smokeMaterial){_disposeMaterialTextures(system.smokeMaterial); system.smokeMaterial.dispose();}} }); } catch(e){error("Cleanup particles error:",e);} hitSparkSystem=null;rainSystem=null;dustSystem=null;campfireSystem=null;
        try { [playerBulletMesh, enemyBulletMesh, ammoCasingMesh].forEach(mesh => { if (mesh) { scene?.remove(mesh); mesh.geometry?.dispose(); } }); } catch(e){error("Cleanup instanced mesh error:",e);} playerBulletMesh=null;enemyBulletMesh=null;ammoCasingMesh=null;
        ...
        try { [playerGroupMap, enemyGroupMap, powerupGroupMap].forEach(objectMap => { for (const id in objectMap) { _handleObjectRemoval(objectMap[id], id, objectMap); } }); } catch(e){error("Cleanup dynamic objects error:",e);}
        try { Object.values(sharedGeometries).forEach(g => g?.dispose()); Object.values(powerupGeometries).forEach(g => g?.dispose()); Object.values(sharedMaterials).forEach(m => { if (m instanceof THREE.Material) { _disposeMaterialTextures(m); m.dispose(); } }); if (sharedMaterials.powerups) Object.values(sharedMaterials.powerups).forEach(m => { if (m instanceof THREE.Material) { _disposeMaterialTextures(m); m.dispose(); } }); Object.values(loadedAssets).forEach(a => a?.dispose()); } catch(e){error("Cleanup shared resources error:",e);}
        ...
        try { if (scene) { if (ambientLight) scene.remove(ambientLight); if (directionalLight) scene.remove(directionalLight); if (directionalLight?.target) scene.remove(directionalLight.target); if (muzzleFlashLight) scene.remove(muzzleFlashLight); } } catch(e){error("Cleanup lights error:",e);} ambientLight=null;directionalLight=null;muzzleFlashLight=null;
        try { if(renderer){console.log("Renderer: Disposing WebGL context...");renderer.dispose();if(renderer.domElement?.parentNode)renderer.domElement.parentNode.removeChild(renderer.domElement);renderer=null;console.log("Renderer disposed.")} } catch(e){error("Cleanup renderer error:",e);}
        scene=null;camera=null;clock=null;domContainer=null;playerBulletMatrices=[];enemyBulletMatrices=[];activeAmmoCasings=[];shakeMagnitude=0;shakeEndTime=0;

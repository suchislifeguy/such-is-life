...
// Note: SkeletonUtils needed if you use complex skinned models later
...
const Renderer3D = (() => {
    ...
    let gameWidth = 1600, gameHeight = 900; // Default, updated from appState
    ...
    const groundGeometryPlane = new THREE.PlaneGeometry(1, 1); groundGeometryPlane.name = "GroundGeo"; // Start with 1x1, scale later
    ...
    const CAMPFIRE_LOG_Y_OFFSET = DEF_LOG_RADIUS; const FLAME_PARTICLE_Y_OFFSET = DEF_LOG_RADIUS + 2; // Flames start slightly above logs
    ...
    function init(containerElement, initialWidth, initialHeight) {
        ...
        try {
            ...
            renderer.outputColorSpace = THREE.SRGBColorSpace; // Correct colorspace
            ...
            directionalLight = new THREE.DirectionalLight(0xffffff, 1.0); directionalLight.position.set(gameWidth * 0.2, 300, gameHeight * 0.3); // Position relative to game size
            ...
            directionalLight.shadow.camera.left = -gameWidth / 2 - 50; directionalLight.shadow.camera.right = gameWidth / 2 + 50;
            directionalLight.shadow.camera.top = gameHeight / 2 + 50; directionalLight.shadow.camera.bottom = -gameHeight / 2 - 50;
            scene.add(directionalLight); scene.add(directionalLight.target); // Target still needed
            ...
            groundPlane.scale.set(gameWidth * 1.1, gameHeight * 1.1, 1); // Scale plane after creation
            ...
            if (containerElement) containerElement.innerHTML = "<p style='color:red; text-align: center;'>Error initializing 3D graphics. Please ensure your browser supports WebGL and try refreshing.</p>";
    ...
    function handleResize() {
        ...
        camera.position.set(gameWidth / 2, 1000, gameHeight / 2); camera.lookAt(gameWidth / 2, 0, gameHeight / 2);
        camera.updateProjectionMatrix(); console.log(`Renderer resized to: ${gameWidth}x${gameHeight}`);
     }

    // --- Entity Creation Functions ---
    function createPlayerMesh(playerData) { const m=new THREE.Mesh(playerGeometry,playerMaterialBase.clone()); m.castShadow=true; m.receiveShadow=true; m.position.set(playerData.x,PLAYER_MESH_Y_OFFSET,playerData.y); m.userData.gameId=playerData.id; m.name = `Player_${playerData.id.substring(0,4)}`; return m; }
    ...
    function updateEnvironment(isNight) {
        ...
        // Simple Fog effect
        if (isNight) { scene.fog = new THREE.FogExp2(0x050a15, 0.0012); }
        else { scene.fog = null; } // No fog during day
    }
    ...
    function getScreenPosition(worldPosition, cameraRef, rendererRef) {
        ...
        const screenX=Math.round((vector.x+1)*rendererRef.domElement.width/2); const screenY=Math.round((-vector.y+1)*rendererRef.domElement.height/2);

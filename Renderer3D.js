...
const Renderer3D = (() => {
    console.log("--- Renderer3D.js Rewrite v3: Initializing ---");
    ...
    const playerMeshes = {};
    const enemyMeshes = {};
    // Bullet rendering now uses InstancedMesh
    const powerupMeshes = {};
    ...
    let playerBulletMesh = null; // InstancedMesh for player bullets
    let enemyBulletMesh = null; // InstancedMesh for enemy bullets
    ...
    const MAX_BULLETS = 200; // Max simultaneous bullets for instancing
    ...
    const bulletPlayerMaterial = new THREE.MeshBasicMaterial({ color: 0xffed4a, name: 'BulletPlayerMat' }); // Used for InstancedMesh now
    const bulletEnemyMaterial = new THREE.MeshBasicMaterial({ color: 0xff4500, name: 'BulletEnemyMat' }); // Used for InstancedMesh now
    ...
    const groundDayMaterial = new THREE.MeshStandardMaterial({ color: 0x697a68, roughness: 0.9, metalness: 0.05, name: 'GroundDayMat' }); // Slightly adjusted color
    ...
    const bulletGeometry = new THREE.SphereGeometry(DEF_BULLET_RADIUS, 6, 6); bulletGeometry.name = "BulletGeo"; // Base geometry for instancing
    ...
    function createFlameTexture() { const c = document.createElement('canvas'); c.width=64; c.height=64; const ctx = c.getContext('2d'); const g = ctx.createRadialGradient(32,32,0,32,32,32); g.addColorStop(0,'rgba(255,220,150,1)'); g.addColorStop(0.4,'rgba(255,150,0,0.8)'); g.addColorStop(1,'rgba(200,0,0,0)'); ctx.fillStyle=g; ctx.fillRect(0,0,64,64); const t = new THREE.CanvasTexture(c); t.name="FlameTexture"; return t; }
    ...
    function init(containerElement, initialWidth, initialHeight) {
        ...
        gameWidth = initialWidth || DEFAULT_CANVAS_WIDTH; gameHeight = initialHeight || DEFAULT_CANVAS_HEIGHT;
        ...
        try {
            ...
            directionalLight.shadow.camera.far = 1200; // Increased far plane for larger maps potentially
            directionalLight.shadow.camera.left = -gameWidth * 0.8; // Adjusted bounds
            directionalLight.shadow.camera.right = gameWidth * 0.8;
            directionalLight.shadow.camera.top = gameHeight * 0.8;
            directionalLight.shadow.camera.bottom = -gameHeight * 0.8;
            ...
            initParticlesAndInstances(); // Combines particle and instanced mesh setup
            initCampfire();
            initSnake();
    ...
    function initParticlesAndInstances() {
        ...
        playerBulletMesh = new THREE.InstancedMesh(bulletGeometry, bulletPlayerMaterial, MAX_BULLETS); playerBulletMesh.name = "PlayerBullets"; playerBulletMesh.count = 0; scene.add(playerBulletMesh);
        enemyBulletMesh = new THREE.InstancedMesh(bulletGeometry, bulletEnemyMaterial, MAX_BULLETS); enemyBulletMesh.name = "EnemyBullets"; enemyBulletMesh.count = 0; scene.add(enemyBulletMesh);
    }
    ...
    function initSnake() { const initialCurve = new THREE.LineCurve3(new THREE.Vector3(0, DEF_SNAKE_RADIUS, 0), new THREE.Vector3(1, DEF_SNAKE_RADIUS, 0)); const snakeGeometry = new THREE.TubeGeometry(initialCurve, DEF_SNAKE_SEGMENTS, DEF_SNAKE_RADIUS, 5, false); snakeGeometry.name = "SnakeGeo"; snakeMesh = new THREE.Mesh(snakeGeometry, snakeMaterial); snakeMesh.name = "Snake"; snakeMesh.castShadow = true; snakeMesh.visible = false; scene.add(snakeMesh); }
    ...
    function createPlayerMesh(playerData) { const playerGroup = new THREE.Group(); playerGroup.name = `Player_${playerData.id.substring(0,4)}`; const bodyMat = playerMaterial.clone(); bodyMat.name = `PMat_${playerData.id.substring(0,4)}`; const bodyMesh = new THREE.Mesh(playerBodyGeometry, bodyMat); bodyMesh.castShadow = true; bodyMesh.position.y = PLAYER_BODY_Y_OFFSET; bodyMesh.name = "PBody"; playerGroup.add(bodyMesh); const headMesh = new THREE.Mesh(headGeometry, headMaterial.clone()); headMesh.scale.setScalar(DEF_PLAYER_HEAD_RADIUS); headMesh.position.y = PLAYER_BODY_Y_OFFSET + (DEF_PLAYER_HEIGHT - DEF_PLAYER_WIDTH)/2 + DEF_PLAYER_HEAD_RADIUS*0.8; headMesh.castShadow = true; headMesh.name = "PHead"; playerGroup.add(headMesh); const gunMesh = new THREE.Mesh(gunGeometry, gunMaterial.clone()); gunMesh.position.set(0, PLAYER_BODY_Y_OFFSET * 0.8, DEF_PLAYER_WIDTH * 0.5); gunMesh.rotation.x = Math.PI / 2; gunMesh.castShadow = true; gunMesh.name = "PGun"; playerGroup.add(gunMesh); playerGroup.position.set(playerData.x, PLAYER_Y_OFFSET, playerData.y); playerGroup.userData.gameId = playerData.id; playerGroup.userData.bodyMesh = bodyMesh; playerGroup.userData.headMesh = headMesh; playerGroup.userData.gunMesh = gunMesh; return playerGroup; }
    function createEnemyMesh(enemyData){ const enemyGroup = new THREE.Group(); enemyGroup.name = `Enemy_${enemyData.type}_${enemyData.id.substring(0,4)}`; let bodyMesh, headMesh, yOffset=ENEMY_Y_OFFSET, bodyMat, headScale = DEF_ENEMY_HEAD_RADIUS; const enemyHeadMat = headMaterial.clone(); if(enemyData.type==='giant'){ bodyMat=enemyGiantMaterial.clone(); bodyMesh=new THREE.Mesh(enemyGiantGeometry,bodyMat); yOffset=GIANT_Y_OFFSET; headScale = DEF_ENEMY_HEAD_RADIUS * DEF_GIANT_MULTIPLIER * 0.8; } else if(enemyData.type==='shooter'){ bodyMat=enemyShooterMaterial.clone(); bodyMesh=new THREE.Mesh(enemyShooterGeometry,bodyMat); const enemyGun = new THREE.Mesh(enemyGunGeometry, gunMaterial.clone()); enemyGun.position.set(0, yOffset * 0.6, DEF_ENEMY_WIDTH * 0.6); enemyGun.rotation.x = Math.PI / 2; enemyGun.castShadow = true; enemyGun.name="EGun"; enemyGroup.add(enemyGun); } else{ bodyMat=enemyChaserMaterial.clone(); bodyMesh=new THREE.Mesh(enemyChaserGeometry,bodyMat); } bodyMesh.castShadow=true; bodyMesh.receiveShadow=true; bodyMesh.position.y = yOffset; bodyMesh.name = "EBody"; headMesh = new THREE.Mesh(headGeometry, enemyHeadMat); headMesh.scale.setScalar(headScale); headMesh.castShadow = true; headMesh.name = "EHead"; headMesh.position.y = yOffset + (bodyMesh.geometry.parameters.height || DEF_ENEMY_HEIGHT)/2 + headScale * 0.7; enemyGroup.add(bodyMesh); enemyGroup.add(headMesh); enemyGroup.position.set(enemyData.x, 0, enemyData.y); enemyGroup.userData.gameId=enemyData.id; enemyGroup.userData.type=enemyData.type; enemyGroup.userData.bodyMesh = bodyMesh; return enemyGroup; }
    function createPowerupMesh(powerupData){ const powerupGroup = new THREE.Group(); powerupGroup.name = `Powerup_${powerupData.type}_${powerupData.id.substring(0,4)}`; const mat = (powerupMaterials[powerupData.type] || powerupMaterials.default).clone(); let iconGeo; switch(powerupData.type) { case 'health': iconGeo = powerupHealthGeo; break; case 'gun_upgrade': iconGeo = powerupGunGeo; break; case 'speed_boost': iconGeo = powerupSpeedGeo; break; case 'armor': iconGeo = powerupArmorGeo; break; case 'ammo_shotgun': iconGeo = powerupShotgunGeo; break; case 'ammo_heavy_slug': iconGeo = powerupSlugGeo; break; case 'ammo_rapid_fire': iconGeo = powerupRapidGeo; break; case 'bonus_score': iconGeo = powerupScoreGeo; break; default: iconGeo = powerupBoxGeometry; } const iconMesh = new THREE.Mesh(iconGeo, mat); iconMesh.castShadow = true; iconMesh.receiveShadow = true; iconMesh.position.y = POWERUP_Y_OFFSET; iconMesh.rotation.set(Math.PI / 6, 0, Math.PI / 6); powerupGroup.add(iconMesh); powerupGroup.position.set(powerupData.x, 0, powerupData.y); powerupGroup.userData.gameId=powerupData.id; powerupGroup.userData.isPowerup=true; powerupGroup.userData.iconMesh = iconMesh; return powerupGroup; }
    ...
    function updateMeshes(state, meshDict, createFn, defaultYPosFn, localEffects, appState) {
        ...
        if (state) {
            for (const id in state) {
                ...
                const bodyMesh = (obj instanceof THREE.Group) ? obj.userData.bodyMesh : null;
                ...
                if (bodyMesh && bodyMesh.material && !isNew) {
                    ...
                    if (meshDict === enemyMeshes && data.health <= 0 && data.death_timestamp) { const timeSinceDeath = (now/1000) - data.death_timestamp; const opacity = Math.max(0, 1.0 - (timeSinceDeath / fadeDuration)); const isVisible = opacity > 0.01; obj.visible = isVisible; obj.children.forEach(c => { if(c.material) { c.material.opacity = opacity; c.material.needsUpdate = true; } c.visible = isVisible; }); }
                    else if (obj.visible === false || (bodyMesh.material.opacity < 1.0 && bodyMesh.material.opacity > 0)) { obj.visible = true; obj.children.forEach(c => { if(c.material) { c.material.opacity = 1.0; c.material.needsUpdate = true; } c.visible = true; }); }
                    const snakeEffect = data.effects?.snake_bite_slow; const isBitten = snakeEffect && now < snakeEffect.expires_at * 1000; const isPushback = meshDict === playerMeshes && id === appState?.localPlayerId && localEffects?.pushbackAnim?.active; let emissiveColor = 0x000000; let emissiveIntensity = 0;
    ...
    function updateBulletInstances(bulletsState) {
        let playerBulletCount = 0;
        let enemyBulletCount = 0;
        if (playerBulletMesh && enemyBulletMesh && bulletsState) {
            for(const id in bulletsState) {
                const bullet = bulletsState[id];
                const targetMesh = bullet.owner_type === 'player' ? playerBulletMesh : enemyBulletMesh;
                let countRef = bullet.owner_type === 'player' ? playerBulletCount : enemyBulletCount;

                if (countRef < MAX_BULLETS) {
                    dummyObject.position.set(bullet.x, BULLET_Y_OFFSET, bullet.y);
                    dummyObject.updateMatrix();
                    targetMesh.setMatrixAt(countRef, dummyObject.matrix);
                    if (bullet.owner_type === 'player') playerBulletCount++;
                    else enemyBulletCount++;
                }
            }
            playerBulletMesh.count = playerBulletCount;
            enemyBulletMesh.count = enemyBulletCount;
    ...
    function updateEnvironment(isNight) { const dayLightIntensity=1.0, nightLightIntensity=0.5; const dayAmbientIntensity=0.6, nightAmbientIntensity=0.4; const dayDirColor=0xffffff, nightDirColor=0xc0d0ff; const dayAmbColor=0xffffff, nightAmbColor=0x6070a0; ambientLight.intensity = isNight ? nightAmbientIntensity : dayAmbientIntensity; ambientLight.color.setHex(isNight ? nightAmbColor : dayAmbColor); directionalLight.intensity = isNight ? nightLightIntensity : dayLightIntensity; directionalLight.color.setHex(isNight ? nightDirColor : dayDirColor); groundPlane.material = isNight ? groundNightMaterial : groundDayMaterial; scene.fog = isNight ? new THREE.FogExp2(0x081020, 0.0008) : null; }
    ...
    function updateSnake(snakeData) { if (!snakeMesh || !snakeData || !snakeMesh.geometry) return; snakeMesh.visible = snakeData.isActiveFromServer; if(snakeData.isActiveFromServer && snakeData.segments && snakeData.segments.length > 1){ snakeCurvePoints = snakeData.segments.map(seg => new THREE.Vector3(seg.x, DEF_SNAKE_RADIUS, seg.y)); if (snakeCurvePoints.length >= 2) { const curve = new THREE.CatmullRomCurve3(snakeCurvePoints); const tubeGeometry = snakeMesh.geometry; const positions = tubeGeometry.attributes.position.array; const P = new THREE.Vector3(); const N = new THREE.Vector3(); const B = new THREE.Vector3(); const vertex = new THREE.Vector3(); const normal = new THREE.Vector3(); const frames = curve.computeFrenetFrames(DEF_SNAKE_SEGMENTS, false); const segments = tubeGeometry.parameters.tubularSegments || DEF_SNAKE_SEGMENTS; const radiusSegments = tubeGeometry.parameters.radialSegments || 5; let vertexIndex = 0; for ( let i = 0; i <= segments; i ++ ) { curve.getPointAt(i / segments, P); const frameNormal = frames.normals[ i ]; const frameBinormal = frames.binormals[ i ]; for ( let j = 0; j <= radiusSegments; j ++ ) { const v = j / radiusSegments * Math.PI * 2; const sin = Math.sin( v ); const cos = - Math.cos( v ); normal.x = ( cos * frameNormal.x + sin * frameBinormal.x ); normal.y = ( cos * frameNormal.y + sin * frameBinormal.y ); normal.z = ( cos * frameNormal.z + sin * frameBinormal.z ); normal.normalize(); vertex.copy(P).addScaledVector(normal, DEF_SNAKE_RADIUS); positions[vertexIndex++] = vertex.x; positions[vertexIndex++] = vertex.y; positions[vertexIndex++] = vertex.z; } } tubeGeometry.attributes.position.needsUpdate = true; tubeGeometry.computeVertexNormals(); } else { snakeMesh.visible = false; } } else { snakeMesh.visible = false; } }
    ...
    function cleanup() {
        ...
        if(snakeMesh?.geometry) snakeMesh.geometry.dispose(); ammoCasingMesh = null; playerBulletMesh = null; enemyBulletMesh = null;
        ...
        scene = null; camera = null; groundPlane = null; ambientLight = null; directionalLight = null; muzzleFlashLight = null; campfireGroup = null; snakeMesh = null; flameParticles = null; hitSparkParticles = null; rainLines = null; dustParticles = null;

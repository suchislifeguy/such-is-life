// Renderer3D.js - FINAL COMPLETE VERSION (Reverted from Debug)
...
const Renderer3D = (() => {
    ...
    // Materials (Cloned where mutation happens, shared otherwise)
    ...
    function init(containerElement, initialWidth, initialHeight) {
        ...
        handleResize(); // Call once after setup to ensure correct initial size
    ...
    function handleResize() {
        if (!renderer || !camera) return; // Guard against calls before init
        ...
        camera.lookAt(gameWidth / 2, 0, gameHeight / 2); // Ensure lookAt is correct after position change
    ...
    function updateHitSparks(activeSparkEffects, deltaTime) {
        if (!hitSparkParticles||!hitSparkGeometry) return; const positions=hitSparkGeometry.attributes.position.array; const colors=hitSparkGeometry.attributes.color.array; const alphas=hitSparkGeometry.attributes.alpha.array; let visibleSparks=false; let firstInactiveIndex=0; // Optimization: find first inactive slot
        const now = performance.now();
        // Activate new sparks
        for(const enemyId in activeSparkEffects){ const effectEndTime=activeSparkEffects[enemyId]; const enemyMesh=enemyMeshes[enemyId]; if(now < effectEndTime && enemyMesh){ const sparksToSpawn=3+Math.floor(Math.random()*4); const enemyPos=enemyMesh.position; for(let i=0;i<sparksToSpawn;i++){ let foundSlot=false; for(let j=firstInactiveIndex; j < MAX_SPARKS; j++){ const p=sparkData[j]; if(p.life<=0){ p.position.copy(enemyPos); p.position.y+=Math.random()*ENEMY_MESH_Y_OFFSET*1.5; const angle=Math.random()*Math.PI*2; const speed=100+Math.random()*80; p.velocity.set(Math.cos(angle)*speed,(Math.random()-0.3)*speed*0.5,Math.sin(angle)*speed); p.color.setRGB(1,Math.random()*0.5,0); p.alpha=0.9+Math.random()*0.1; p.life=0.2+Math.random()*0.3; firstInactiveIndex=j+1; // Start search from next slot next time
        foundSlot=true; break;}} if (!foundSlot) break; // Stop trying to spawn if no slots left
        } delete activeSparkEffects[enemyId]; } } // Remove effect once processed for this frame
    ...
    function updateSnake(snakeData) {
        ...
        if(snakeData.isActiveFromServer&&snakeData.segments&&snakeData.segments.length>1){
            ...
            if (curvePoints.length >= 2) { const curve=new THREE.CatmullRomCurve3(curvePoints); const newGeometry=new THREE.TubeGeometry(curve,DEF_SNAKE_SEGMENTS,DEF_SNAKE_RADIUS,5,false); newGeometry.name = "SnakeGeo_Updated"; snakeMesh.geometry.dispose(); snakeMesh.geometry=newGeometry; }
    ...
    function renderScene(stateToRender, appState, localEffects) {
        if (!renderer || !scene || !camera || !stateToRender || !appState || !localEffects) { /* console.warn("Render scene skipping: Missing core components"); */ return; } // Reduce console noise

...
const Renderer3D = (() => {
    console.log("--- Renderer3D.js Rewrite v4: Initializing ---");

    let scene, camera, renderer;
    let ambientLight, directionalLight, muzzleFlashLight;
    let gameWidth = 3200; // Increased Default/Max Width
    let gameHeight = 1800; // Increased Default/Max Height
    ...
    let tempBulletMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, name: 'TempBulletMat' }); // For Debugging
    let tempHeadMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00, name: 'TempHeadMat'}); // For Debugging
    ...
    const MAX_BULLETS = 500; // Increased for debugging
    ...
    const DEF_POWERUP_SIZE = 15; const DEF_SNAKE_SEGMENTS = 30; // Segments for curve (lower for geometry recreation)
    ...
    // Original materials kept for reference, but InstancedMesh uses tempBulletMaterial for now
    ...
    const powerupMaterials = { default: new THREE.MeshStandardMaterial({ color: 0x888888, name: 'PUMat_Def' }), health: new THREE.MeshStandardMaterial({ color: 0x81c784, name: 'PUMat_HP' }), gun_upgrade: new THREE.MeshStandardMaterial({ color: 0x6a0dad, emissive: 0x110011, name: 'PUMat_Gun' }), speed_boost: new THREE.MeshStandardMaterial({ color: 0x3edef3, name: 'PUMat_Spd' }), armor: new THREE.MeshStandardMaterial({ color: 0xaaaaaa, metalness: 0.7, name: 'PUMat_Arm' }), ammo_shotgun: new THREE.MeshStandardMaterial({ color: 0xFFa500, name: 'PUMat_Shot' }), ammo_heavy_slug: new THREE.MeshStandardMaterial({ color: 0xA0522D, name: 'PUMat_Slug' }), ammo_rapid_fire: new THREE.MeshStandardMaterial({ color: 0xFFFF00, emissive: 0x333300, name: 'PUMat_Rap' }), bonus_score: new THREE.MeshStandardMaterial({ color: 0xFFD700, metalness: 0.6, name: 'PUMat_Sco' }), };
    const groundDayMaterial = new THREE.MeshStandardMaterial({ color: 0x697a68, roughness: 0.9, metalness: 0.05, name: 'GroundDayMat' });
    const groundNightMaterial = new THREE.MeshStandardMaterial({ color: 0x4E342E, roughness: 0.85, metalness: 0.1, name: 'GroundNightMat' }); // Lighter night ground
    ...
    function init(containerElement, initialWidth, initialHeight) {
        ...
        if (!containerElement) { console.error("No container."); return false; }
    ...
    function initParticlesAndInstances() {
        hitSparkGeometry = new THREE.BufferGeometry(); const sp = new Float32Array(MAX_SPARKS*3); const sc = new Float32Array(MAX_SPARKS*3); const sa = new Float32Array(MAX_SPARKS); sparkData = []; for(let i=0;i<MAX_SPARKS;i++){sparkData.push({p:new THREE.Vector3(0,-1e3,0),v:new THREE.Vector3(),c:new THREE.Color(1,1,1),a:0.0,l:0});sa[i]=0.0;sp[i*3+1]=-1e3;} hitSparkGeometry.setAttribute('position',new THREE.BufferAttribute(sp,3)); hitSparkGeometry.setAttribute('color',new THREE.BufferAttribute(sc,3)); hitSparkGeometry.setAttribute('alpha',new THREE.BufferAttribute(sa,1)); hitSparkParticles = new THREE.Points(hitSparkGeometry, sparkMaterial); scene.add(hitSparkParticles);
        rainGeometry = new THREE.BufferGeometry(); const rp = new Float32Array(MAX_RAIN_DROPS*6); rainData = []; for(let i=0;i<MAX_RAIN_DROPS;i++){rainData.push({p:new THREE.Vector3(Math.random()*gameWidth*1.2-gameWidth*0.1,gameHeight*1.5,Math.random()*gameHeight*1.2-gameHeight*0.1),v:new THREE.Vector3(15+Math.random()*8,-400-Math.random()*150,0)}); } rainGeometry.setAttribute('position',new THREE.BufferAttribute(rp,3)); rainLines = new THREE.LineSegments(rainGeometry, rainMaterial); rainLines.visible = false; scene.add(rainLines);
        dustGeometry = new THREE.BufferGeometry(); const dp = new Float32Array(MAX_DUST_MOTES*3); dustData = []; for(let i=0;i<MAX_DUST_MOTES;i++){dustData.push({p:new THREE.Vector3(Math.random()*gameWidth*1.2-gameWidth*0.1,Math.random()*50+5,Math.random()*gameHeight*1.2-gameHeight*0.1),v:new THREE.Vector3((Math.random()-.5)*50,Math.random()*5,(Math.random()-.5)*50),r:Math.random()*Math.PI*2}); dp[i*3+1]=-1e3;} dustGeometry.setAttribute('position',new THREE.BufferAttribute(dp,3)); dustParticles = new THREE.Points(dustGeometry, dustMaterial); dustParticles.visible = false; scene.add(dustParticles);
        ...
        playerBulletMesh = new THREE.InstancedMesh(bulletGeometry, tempBulletMaterial, MAX_BULLETS); playerBulletMesh.count = 0; scene.add(playerBulletMesh); // Using TEMP material for debug
        enemyBulletMesh = new THREE.InstancedMesh(bulletGeometry, tempBulletMaterial, MAX_BULLETS); enemyBulletMesh.count = 0; scene.add(enemyBulletMesh); // Using TEMP material for debug
    }

    function initCampfire() { campfireGroup = new THREE.Group(); const l1=new THREE.Mesh(logGeometry,logMaterial);l1.rotation.set(0,Math.PI/10,Math.PI/6);l1.castShadow=true;l1.position.set(-DEF_LOG_LENGTH*.1,CAMPFIRE_LOG_Y_OFFSET,-DEF_LOG_LENGTH*.2); const l2=new THREE.Mesh(logGeometry,logMaterial);l2.rotation.set(0,-Math.PI/8,-Math.PI/5);l2.castShadow=true;l2.position.set(DEF_LOG_LENGTH*.15,CAMPFIRE_LOG_Y_OFFSET,DEF_LOG_LENGTH*.1); campfireGroup.add(l1);campfireGroup.add(l2); const g=new THREE.PointLight(0xffa500,0,150,1.8);g.position.y=FLAME_PARTICLE_Y_OFFSET+5;campfireGroup.add(g);campfireGroup.userData.glowLight=g; flameGeometry=new THREE.BufferGeometry();const fp=new Float32Array(MAX_FLAMES*3);const fc=new Float32Array(MAX_FLAMES*3);const fa=new Float32Array(MAX_FLAMES);flameData=[];for(let i=0;i<MAX_FLAMES;i++){flameData.push({p:new THREE.Vector3(0,-1e3,0),v:new THREE.Vector3(),c:new THREE.Color(1,1,1),a:0.0,l:0,baseLife:.5+Math.random()*.4});fp[i*3+1]=-1e3;fa[i]=0.0;}flameGeometry.setAttribute('position',new THREE.BufferAttribute(fp,3));flameGeometry.setAttribute('color',new THREE.BufferAttribute(fc,3));flameGeometry.setAttribute('alpha',new THREE.BufferAttribute(fa,1));flameParticles=new THREE.Points(flameGeometry,flameMaterial);campfireGroup.add(flameParticles);campfireGroup.userData.flameParticles=flameParticles;campfireGroup.visible=false;scene.add(campfireGroup); }
    function initSnake() { const c = new THREE.LineCurve3(new THREE.Vector3(0, DEF_SNAKE_RADIUS, 0), new THREE.Vector3(1, DEF_SNAKE_RADIUS, 0)); const g = new THREE.TubeGeometry(c, 10, DEF_SNAKE_RADIUS, 5, false); snakeMesh = new THREE.Mesh(g, snakeMaterial); snakeMesh.castShadow = true; snakeMesh.visible = false; scene.add(snakeMesh); } // Init with dummy geometry
    ...
    function createPlayerMesh(d) { const g=new THREE.Group(); const bm=new THREE.Mesh(playerBodyGeometry,playerMaterial.clone());bm.castShadow=true;bm.position.y=PLAYER_BODY_Y_OFFSET;g.add(bm);const hm=new THREE.Mesh(headGeometry,tempHeadMaterial.clone());hm.scale.setScalar(DEF_PLAYER_HEAD_RADIUS);hm.position.y=PLAYER_BODY_Y_OFFSET+(DEF_PLAYER_HEIGHT-DEF_PLAYER_WIDTH)/2+DEF_PLAYER_HEAD_RADIUS*.8;hm.castShadow=true;g.add(hm);const gm=new THREE.Mesh(gunGeometry,gunMaterial.clone());gm.position.set(0,PLAYER_BODY_Y_OFFSET*.8,DEF_PLAYER_WIDTH*.5);gm.rotation.x=Math.PI/2;gm.castShadow=true;g.add(gm);g.position.set(d.x,PLAYER_Y_OFFSET,d.y);g.userData={gameId:d.id,bodyMesh:bm,headMesh:hm,gunMesh:gm};return g; }
    function createEnemyMesh(d){ const g=new THREE.Group(); let bm,hm,yOff=ENEMY_Y_OFFSET,bMat,hScl=DEF_ENEMY_HEAD_RADIUS; const ehMat=tempHeadMaterial.clone(); if(d.type==='giant'){bMat=enemyGiantMaterial.clone();bm=new THREE.Mesh(enemyGiantGeometry,bMat);yOff=GIANT_Y_OFFSET;hScl=DEF_ENEMY_HEAD_RADIUS*DEF_GIANT_MULTIPLIER*.8;}else if(d.type==='shooter'){bMat=enemyShooterMaterial.clone();bm=new THREE.Mesh(enemyShooterGeometry,bMat);const eg=new THREE.Mesh(enemyGunGeometry,gunMaterial.clone());eg.position.set(0,yOff*.6,DEF_ENEMY_WIDTH*.6);eg.rotation.x=Math.PI/2;eg.castShadow=true;g.add(eg);}else{bMat=enemyChaserMaterial.clone();bm=new THREE.Mesh(enemyChaserGeometry,bMat);} bm.castShadow=true;bm.receiveShadow=true;bm.position.y=yOff;hm=new THREE.Mesh(headGeometry,ehMat);hm.scale.setScalar(hScl);hm.castShadow=true;hm.position.y=yOff+(bm.geometry.parameters.height||DEF_ENEMY_HEIGHT)/2+hScl*.7; g.add(bm);g.add(hm);g.position.set(d.x,0,d.y);g.userData={gameId:d.id,type:d.type,bodyMesh:bm};return g; }
    ...
    function updateMeshes(state, meshDict, createFn, defaultYPosFn, localEffects, appState) { const activeIds = new Set(); if (state) { for (const id in state) { const data = state[id]; if (typeof data.x !== 'number' || typeof data.y !== 'number') continue; activeIds.add(id); let obj = meshDict[id]; let isNew = false; if (!obj) { obj = createFn(data); if (obj) { meshDict[id] = obj; scene.add(obj); isNew = true; } else { continue; } } const yOffset = defaultYPosFn(data); obj.position.set(data.x, yOffset, data.y); const bodyMesh = (obj instanceof THREE.Group) ? obj.userData.bodyMesh : obj; const iconMesh = (obj instanceof THREE.Group && obj.userData.isPowerup) ? obj.userData.iconMesh : null; if (bodyMesh && bodyMesh.material && !isNew) { const fadeDuration = 0.4; const now = performance.now(); if (meshDict === enemyMeshes && data.health <= 0 && data.death_timestamp) { const timeSinceDeath = (now/1000) - data.death_timestamp; const opacity = Math.max(0, 1.0 - (timeSinceDeath / fadeDuration)); const isVisible = opacity > 0.01; obj.visible = isVisible; obj.children.forEach(c => { if(c.material) { c.material.opacity = opacity; c.material.needsUpdate = true; } c.visible = isVisible; }); } else if (obj.visible === false || (bodyMesh.material.opacity < 1.0 && bodyMesh.material.opacity > 0)) { obj.visible = true; obj.children.forEach(c => { if(c.material) { c.material.opacity = 1.0; c.material.needsUpdate = true; } c.visible = true; }); } const snakeEffect = data.effects?.snake_bite_slow; const isBitten = snakeEffect && now < snakeEffect.expires_at * 1000; const isPushback = meshDict === playerMeshes && id === appState?.localPlayerId && localEffects?.pushbackAnim?.active; let emissiveColor = 0x000000; let emissiveIntensity = 0; if(isPushback) { const pushbackProgress = Math.max(0,(localEffects.pushbackAnim.endTime-now)/localEffects.pushbackAnim.duration); emissiveIntensity = Math.sin(pushbackProgress*Math.PI)*0.8; emissiveColor = 0x66ccff; } else if (isBitten) { emissiveIntensity = 0.4 + Math.sin(now * 0.005) * 0.3; emissiveColor = 0x00ff00; } if(bodyMesh.material.emissive) { bodyMesh.material.emissive.setHex(emissiveColor); bodyMesh.material.emissiveIntensity = emissiveIntensity; } } if (iconMesh) { iconMesh.rotation.y += 0.02; iconMesh.position.y = POWERUP_Y_OFFSET + Math.sin(performance.now() * 0.002) * 3; } if (meshDict === playerMeshes && obj.userData?.gunMesh) { const aimDx = data.id === appState?.localPlayerId ? appState.localPlayerAimState.lastAimDx : 0; const aimDy = data.id === appState?.localPlayerId ? appState.localPlayerAimState.lastAimDy : -1; obj.rotation.y = Math.atan2(aimDx, aimDy); } else if (meshDict === enemyMeshes && obj.userData?.type === 'shooter') { const targetPlayer = appState?.serverState?.players?.[data.target_player_id]; if (targetPlayer) { const aimTargetX = targetPlayer.x; const aimTargetZ = targetPlayer.y; obj.rotation.y = Math.atan2(aimTargetX - data.x, aimTargetZ - data.y); } } } } for (const id in meshDict) { if (!activeIds.has(id)) { const objToRemove = meshDict[id]; if (objToRemove) { scene.remove(objToRemove); disposeObject3D(objToRemove); } delete meshDict[id]; } } }
    function updateBulletInstances(bulletsState) { let pCount = 0; let eCount = 0; if (playerBulletMesh && enemyBulletMesh && bulletsState) { for(const id in bulletsState) { const bullet = bulletsState[id]; const isPlayer = bullet.owner_type === 'player'; const targetMesh = isPlayer ? playerBulletMesh : enemyBulletMesh; let countRef = isPlayer ? pCount : eCount; if (countRef < MAX_BULLETS) { dummyObject.position.set(bullet.x, BULLET_Y_OFFSET, bullet.y); dummyObject.rotation.set(0,0,0); dummyObject.scale.set(1,1,1); dummyObject.updateMatrix(); targetMesh.setMatrixAt(countRef, dummyObject.matrix); if (isPlayer) pCount++; else eCount++; } } playerBulletMesh.count = pCount; enemyBulletMesh.count = eCount; playerBulletMesh.instanceMatrix.needsUpdate = true; enemyBulletMesh.instanceMatrix.needsUpdate = true; } console.log(`Bullets: P=${pCount}, E=${eCount}`); } // Debug Log
    ...
    function updateEnvironment(isNight) { const dayLI=1.0, nightLI=0.55; const dayAI=0.6, nightAI=0.45; const dayDC=0xffffff, nightDC=0xc0d0ff; const dayAC=0xffffff, nightAC=0x7080b0; ambientLight.intensity=isNight?nightAI:dayAI; ambientLight.color.setHex(isNight?nightAC:dayAC); directionalLight.intensity=isNight?nightLI:dayLI; directionalLight.color.setHex(isNight?nightDC:dayDC); groundPlane.material=isNight?groundNightMaterial:groundDayMaterial; scene.fog = isNight?new THREE.FogExp2(0x081020, 0.0008):null; }
    ...
    function cleanup() {
        ...
        Object.values(powerupMaterials).forEach(m => m.dispose()); playerMaterial.dispose(); enemyChaserMaterial.dispose(); enemyShooterMaterial.dispose(); enemyGiantMaterial.dispose(); snakeMaterial.dispose(); logMaterial.dispose(); gunMaterial.dispose(); headMaterial.dispose(); tempHeadMaterial.dispose();

...
function _createEnemyGroup(enemyData) {
    ...
    switch (enemyData.type) {
        case 'shooter':
            bodyGeo = sharedGeometries.enemyShooterBody; bodyMat = sharedMaterials.enemyShooterBody; headScale = ENEMY_HEAD_RADIUS; yBodyOffset = enemyHeight / 2;
            ...
            break;
        case 'giant':
            bodyGeo = sharedGeometries.enemyGiantBody; bodyMat = sharedMaterials.enemyGiantBody; headScale = ENEMY_HEAD_RADIUS * ENEMY_GIANT_MULTIPLIER * 0.8; yBodyOffset = enemyHeight / 2;
            break;
        case 'chaser': default:
            bodyGeo = sharedGeometries.enemyChaserBody; bodyMat = sharedMaterials.enemyChaserBody; headScale = ENEMY_HEAD_RADIUS; yBodyOffset = enemyHeight / 2;
            break;
    }
    ...
    group.userData = { gameId: enemyData.id, isEnemy: true, type: enemyData.type, bodyMesh: bodyMesh, headMesh: headMesh, gunMesh: gunMesh, currentBodyMatRef: bodyMat, dyingStartTime: null, health: enemyData.health };
    return group;
}
...
function _disposeMaterialTextures(material) {
    ...
    textures.forEach(prop => { if (material[prop] && material[prop] instanceof THREE.Texture) material[prop].dispose(); });
}
        ...
        if (child instanceof THREE.Mesh || child instanceof THREE.Points || child instanceof THREE.LineSegments) {
            ...
            materials.forEach(m => {
                if (m) {
                    ...
                    if (!isShared && sharedMaterials.powerups) { isShared = Object.values(sharedMaterials.powerups).includes(m); } // Fixed check
                    if (!isShared) { _disposeMaterialTextures(m); m.dispose(); }
                }
            });
        } else if (child instanceof THREE.PointLight && child !== muzzleFlashLight && child !== campfireSystem?.glowLight) { /* No standard dispose */ }
    });
}

function _disposeAndRemoveObject(obj, id, objectMap) {
    if (!obj || !scene) return;
    scene.remove(obj); _disposeObject3D(obj); delete objectMap[id];
}

function _handleObjectRemoval(obj, id, objectMap) {
    if (!obj || !clock) return;
    const userData = obj.userData; const isEnemy = userData?.isEnemy;
    ...
    if (isEnemy && wasAlive && !userData.dyingStartTime) {
        ...
        obj.traverse(child => { if (child.material) { const materials = Array.isArray(child.material) ? child.material : [child.material]; materials.forEach(m => { m.transparent = true; m.needsUpdate = true; }); } });
...
function _syncSceneObjects(state, objectMap, createFn, updateFn, isSelfFn = null) {
    ...
    if (state) {
        for (const id in state) {
            ...
            if (!obj) {
                const isSelf = isSelfFn ? isSelfFn(id) : false; obj = createFn(data, isSelf);
                if (obj) { objectMap[id] = obj; scene.add(obj); updateFn(obj, data, 0, isSelf); }
            } else {
                ...
                if (obj.userData?.dyingStartTime) {
                    ...
                    obj.traverse(child => { if (child.material) { const materials = Array.isArray(child.material) ? child.material : [child.material]; materials.forEach(m => { m.opacity = 1.0; m.transparent = false; m.needsUpdate = true; }); } });
                }
...
function _updatePlayerGroup(group, playerData, deltaTime, isSelf) {
    ...
    const userData = group.userData; const bodyMesh = userData.bodyMesh;
    let targetMatRef = isSelf ? sharedMaterials.playerSelfBody : sharedMaterials.playerBody;
    switch (playerData.player_status) {
        case PLAYER_STATUS_DOWN: targetMatRef = sharedMaterials.playerDown; group.visible = true; break;
        case PLAYER_STATUS_DEAD: group.visible = false; userData.dyingStartTime = null; return;
        case PLAYER_STATUS_ALIVE: default: group.visible = true; break;
    }
    if (bodyMesh && userData.currentBodyMatRef !== targetMatRef) {
        userData.currentBodyMatRef = targetMatRef; bodyMesh.material = targetMatRef.clone(); bodyMesh.material.needsUpdate = true;
    }
    if (isSelf && window.appState?.localPlayerAimState) {
        const { lastAimDx, lastAimDy } = window.appState.localPlayerAimState; group.rotation.y = Math.atan2(lastAimDx, lastAimDy);
    }
}
...
function _updateEnemyGroup(group, enemyData, deltaTime) {
    ...
    if (enemyData.type === 'shooter' && gunMesh && window.appState?.serverState?.players) {
        ...
        if (targetPlayer) { const dx = targetPlayer.x - enemyData.x; const dz = targetPlayer.y - enemyData.y; group.rotation.y = Math.atan2(dx, dz); }
    }
    const bodyMesh = group.userData.bodyMesh;
    if (bodyMesh && enemyData.type === 'giant') {
        const isWindingUp = enemyData.attack_state === 'winding_up'; const scaleTarget = isWindingUp ? 1.0 + Math.sin(clock.elapsedTime * 10) * 0.05 : 1.0;
...
function _updatePowerupGroup(group, powerupData, deltaTime) {
    ...
    if (iconMesh) {
        iconMesh.position.y = Y_OFFSET_POWERUP + Math.sin(clock.elapsedTime * 2.5 + group.id) * 4; // Fixed: Use numeric ID
...
function _updateInstancedMesh(mesh, matrices, state, yOffset, isBullet = false) {
    if (!mesh) return; if (!state || Object.keys(state).length === 0) { mesh.count = 0; mesh.instanceMatrix.needsUpdate = true; return; }
    let visibleCount = 0; const maxCount = matrices.length / 16;
    for (const id in state) {
        if (visibleCount >= maxCount) { console.warn(`Renderer: Exceeded max instances (${maxCount}) for mesh ${mesh.name}.`); break; }
        const data = state[id];
        if (isBullet) { const isPlayerBullet = data.owner_type === 'player'; if (mesh === playerBulletMesh && !isPlayerBullet) continue; if (mesh === enemyBulletMesh && isPlayerBullet) continue; }
        if (typeof data?.x !== 'number' || typeof data?.y !== 'number') continue;
        _position.set(data.x, yOffset, data.y);
        if (isBullet && data.vx !== undefined && data.vy !== undefined && (data.vx !== 0 || data.vy !== 0)) { const angle = Math.atan2(data.vx, data.vy); _quaternion.setFromEuler(new THREE.Euler(0, angle, 0)); } else { _quaternion.identity(); }
         _scale.set(1, 1, 1); _matrix.compose(_position, _quaternion, _scale); mesh.setMatrixAt(visibleCount, _matrix); visibleCount++;
    }
    mesh.count = visibleCount; mesh.instanceMatrix.needsUpdate = true;
}

function _updateActiveCasings(deltaTime) {
    if (!ammoCasingMesh || !clock) return; if (activeAmmoCasings.length === 0) { if (ammoCasingMesh.count > 0) { ammoCasingMesh.count = 0; ammoCasingMesh.instanceMatrix.needsUpdate = true; } return; }
    const now = clock.elapsedTime; let needsUpdate = false;
    activeAmmoCasings = activeAmmoCasings.filter(casing => {
        if (now > casing.endTime) return false; casing.velocity.y -= AMMO_CASING_GRAVITY * deltaTime; casing.position.addScaledVector(casing.velocity, deltaTime); casing.rotation += casing.rotationSpeed * deltaTime;
        if (casing.position.y <= Y_OFFSET_CASING) {
            casing.position.y = Y_OFFSET_CASING; casing.velocity.y *= -AMMO_CASING_BOUNCE; casing.velocity.x *= (1.0 - AMMO_CASING_DRAG); casing.velocity.z *= (1.0 - AMMO_CASING_DRAG); casing.rotationSpeed *= (1.0 - AMMO_CASING_DRAG * 2.0);
            if (Math.abs(casing.velocity.y) < 5) casing.velocity.y = 0; if(Math.abs(casing.rotationSpeed) < 0.1) casing.rotationSpeed = 0;
        } return true;
    });
    let visibleCount = 0; const maxCount = ammoCasingMesh.instanceMatrix.array.length / 16;
    for (let i = 0; i < activeAmmoCasings.length; i++) {
        if (visibleCount >= maxCount) break; const casing = activeAmmoCasings[i]; _quaternion.setFromEuler(new THREE.Euler(Math.PI / 2, casing.rotation, 0)); _matrix.compose(casing.position, _quaternion, _scale); ammoCasingMesh.setMatrixAt(visibleCount, _matrix); visibleCount++; needsUpdate = true;
    }
...
function _updateHitSparks(deltaTime) {
    ...
    const positions = hitSparkSystem.geometry.attributes.position.array; const colors = hitSparkSystem.geometry.attributes.color.array; const alphas = hitSparkSystem.geometry.attributes.alpha.array; const data = hitSparkSystem.data; let needsGeomUpdate = false; let activeCount = 0;
    for (let i = 0; i < MAX_HIT_SPARKS; i++) {
        const p = data[i]; if (p.l > 0) { p.l -= deltaTime; activeCount++; if (p.l <= 0) { alphas[i] = 0.0; positions[i * 3 + 1] = -10000; } else { p.v.y -= HIT_SPARK_GRAVITY * deltaTime; p.p.addScaledVector(p.v, deltaTime); p.a = Math.min(1.0, Math.max(0, (p.l / (HIT_SPARK_BASE_LIFE + HIT_SPARK_RAND_LIFE)) * 1.5)); alphas[i] = p.a; positions[i*3+0]=p.p.x; positions[i*3+1]=p.p.y; positions[i*3+2]=p.p.z; colors[i*3+0]=p.c.r; colors[i*3+1]=p.c.g; colors[i*3+2]=p.c.b; } needsGeomUpdate = true; } else if (alphas[i] > 0) { alphas[i] = 0.0; positions[i * 3 + 1] = -10000; needsGeomUpdate = true; } }
    if (needsGeomUpdate) { hitSparkSystem.geometry.attributes.position.needsUpdate = true; hitSparkSystem.geometry.attributes.color.needsUpdate = true; hitSparkSystem.geometry.attributes.alpha.needsUpdate = true; }
...
function _triggerHitSparks(position, count = 5) {
    ...
    if (spawned > 0) hitSparkSystem.particles.visible = true;
}
...
function _updateRain(deltaTime) {
    ...
    const currentWidth = currentCanvasWidth * GROUND_MARGIN; const currentHeight = currentCanvasHeight * GROUND_MARGIN;
    const widthOffset = (currentCanvasWidth * (GROUND_MARGIN - 1)) / 2; const heightOffset = (currentCanvasHeight * (GROUND_MARGIN - 1)) / 2;
    for (let i = 0; i < MAX_RAIN_DROPS; i++) {
        const p = data[i]; p.y += p.s * deltaTime;
        if (p.y < -50) { p.x = Math.random() * currentWidth - widthOffset; p.y = Math.random() * 500 + 1000; p.z = Math.random() * currentHeight - heightOffset; p.s = RAIN_SPEED_Y + Math.random() * RAIN_SPEED_Y_RAND; }
...
function _updateDust(deltaTime) {
     ...
     const worldWidth = currentCanvasWidth * GROUND_MARGIN; const worldHeight = currentCanvasHeight * GROUND_MARGIN; const halfW = currentCanvasWidth / 2; const halfH = currentCanvasHeight / 2; const marginW = (currentCanvasWidth * (GROUND_MARGIN - 1)) / 2; const marginH = (currentCanvasHeight * (GROUND_MARGIN - 1)) / 2;
     for (let i = 0; i < MAX_DUST_MOTES; i++) {
         const p = data[i]; p.p.addScaledVector(p.v, deltaTime);
         if(p.p.x < halfW - marginW) p.p.x += worldWidth; else if(p.p.x > halfW + worldWidth - marginW) p.p.x -= worldWidth; if(p.p.z < halfH - marginH) p.p.z += worldHeight; else if(p.p.z > halfH + worldHeight - marginH) p.p.z -= worldHeight;
         p.p.y += (Math.random()-0.5)*DUST_SPEED_Y*deltaTime; p.p.y=Math.max(5,Math.min(80, p.p.y)); positions[i*3+0]=p.p.x; positions[i*3+1]=p.p.y; positions[i*3+2]=p.p.z; needsUpdate = true;
...
function _updateCampfire(deltaTime) {
    ...
    const positions = campfireSystem.geometry.attributes.position.array; const colors = campfireSystem.geometry.attributes.color.array; const alphas = campfireSystem.geometry.attributes.alpha.array; const data = campfireSystem.data; let needsGeomUpdate = false; let activeCount = 0;
    ...
    if (needsGeomUpdate) { campfireSystem.geometry.attributes.position.needsUpdate = true; campfireSystem.geometry.attributes.color.needsUpdate = true; campfireSystem.geometry.attributes.alpha.needsUpdate = true; }
    if (campfireSystem.glowLight) campfireSystem.glowLight.intensity = 2.5 + Math.sin(clock.elapsedTime * 3.0) * 0.8;
    if(campfireSystem.group && window.appState?.serverState?.campfire) { const cfData = window.appState.serverState.campfire; campfireSystem.group.position.set(cfData.x, 0, cfData.y); campfireSystem.group.visible = cfData.active; campfireSystem.particles.visible = cfData.active; } else if (campfireSystem.group) { campfireSystem.group.visible = false; campfireSystem.particles.visible = false; }
}

function _updateSnake(snakeData) {
    if (!snakeMesh) { return; } const isActive = snakeData?.active ?? false; snakeMesh.visible = isActive;
    if (isActive && snakeData.segments && snakeData.segments.length > 1) {
        ...
        if (points.length >= 2) {
            try { const curve = new THREE.CatmullRomCurve3(points, false, 'catmullrom', 0.1); const tubePoints = curve.getPoints(SNAKE_VISUAL_SEGMENTS * 2);
                if(tubePoints.length >= 2) { const newGeometry = new THREE.TubeGeometry( new THREE.CatmullRomCurve3(tubePoints), tubePoints.length - 1, SNAKE_RADIUS, 6, false ); snakeMesh.geometry.dispose(); snakeMesh.geometry = newGeometry; snakeMesh.visible = true; } else snakeMesh.visible = false;
            } catch(e) { console.error("Renderer Error updating snake geometry:", e); snakeMesh.visible = false; }
...
function _updateEnvironment(isNight, isRaining, isDustStorm) {
     ...
     if (rainSystem?.lines) rainSystem.lines.visible = isRaining; if (dustSystem?.particles) dustSystem.particles.visible = isDustStorm;
}

function _updateMuzzleFlash(localEffects, playerGroup) {
    if (!muzzleFlashLight || !clock) return; const flashState = localEffects?.muzzleFlash; const nowMs = clock.elapsedTime * 1000;
    if (flashState?.active && nowMs < flashState.endTime && playerGroup) {
        muzzleFlashLight.intensity = 5.0 + Math.random() * 4.0; const gunMesh = playerGroup.userData.gunMesh;
        if (gunMesh) { _vector3.set(0, 0, PLAYER_GUN_LENGTH / 2 + 5); gunMesh.localToWorld(_vector3); muzzleFlashLight.position.copy(_vector3); } else { muzzleFlashLight.intensity = 0; }
...
function _projectToScreen(worldPosition) {
    ...
    try {
        _vector3.copy(worldPosition); _vector3.project(camera);
        const rect = domContainer.getBoundingClientRect(); const widthHalf = rect.width / 2; const heightHalf = rect.height / 2;
        const screenX = Math.round((_vector3.x * widthHalf) + widthHalf); const screenY = Math.round(-(_vector3.y * heightHalf) + heightHalf);
...
const Renderer3D = {
    init: (containerElement, initialWidth, initialHeight) => {
        ...
        try {
            ...
            directionalLight.position.set(currentCanvasWidth * 0.3, 400, currentCanvasHeight * 0.4);
            ...
            groundPlane.rotation.x = -Math.PI / 2; groundPlane.receiveShadow = true; groundPlane.name = "GroundPlane"; scene.add(groundPlane);
    ...
    handleContainerResize: () => {
        ...
        renderer.setViewport(0, 0, newWidth, newHeight); renderer.setScissor(0, 0, newWidth, newHeight); renderer.setScissorTest(true);
        if (groundPlane) { groundPlane.scale.set(currentCanvasWidth * GROUND_MARGIN, currentCanvasHeight * GROUND_MARGIN, 1); groundPlane.position.set(currentCanvasWidth / 2, 0, currentCanvasHeight / 2); }
        if (directionalLight?.target) { directionalLight.target.position.set(currentCanvasWidth / 2, 0, currentCanvasHeight / 2); }
        if (directionalLight?.shadow?.camera) { const sX = currentCanvasWidth * GROUND_MARGIN * 0.55; const sZ = currentCanvasHeight * GROUND_MARGIN * 0.55; directionalLight.shadow.camera.left = -sX; directionalLight.shadow.camera.right = sX; directionalLight.shadow.camera.top = sZ; directionalLight.shadow.camera.bottom = -sZ; directionalLight.shadow.camera.updateProjectionMatrix(); }
        // console.log(`>>> Resize Handled - Canvas W: ${currentCanvasWidth.toFixed(0)}, H: ${currentCanvasHeight.toFixed(0)}`); // DEBUG
    ...
    triggerShake: (magnitude, durationMs) => { if(!clock)return;const n=clock.elapsedTime*1e3,e=n+durationMs;if(magnitude>=shakeMagnitude||e>shakeEndTime)shakeMagnitude=Math.max(.1,magnitude),shakeEndTime=Math.max(n,e)},
    ...
    cleanup: () => {
        ...
        [playerGroupMap,enemyGroupMap,powerupGroupMap].forEach(o=>{for(const id in o)_disposeAndRemoveObject(o[id],id,o)});

...
const Renderer = (() => {
    ...
    function drawPlayerCharacter( ctx, x, y, w, h, isSelf, playerState, aimDx, aimDy ) { const jh=playerState?.hit_flash_this_tick??false,ii=(playerState?.input_vector?.dx??0)===0&&(playerState?.input_vector?.dy??0)===0,t=performance.now(),bo=ii?Math.sin(t/IDLE_BOB_SPEED_DIVISOR)*IDLE_BOB_AMPLITUDE:0; const hh=h*0.3,hw=w*0.95,slh=hh*0.15,slw=hw*0.8,ngh=h*0.06,spw=w*1.25,sph=h*0.1,cph_local=h*0.3,cpw=w*0.9,aw=w*0.2,al=h*0.4,bh=h*0.05,ph=h*0.35,boh=h*0.1,bow=w*0.32,bos=w*0.4; const to=h*0.5,hty=y-to+bo,hby=hty+hh,sly=hty+hh*0.4,ngty=hby-3,ngby=ngty+ngh,sty=ngby-2,sby=sty+sph,cpty=sty+sph*0.15,aty=sty+sph*0.2,bey=cpty+cph_local+bh*0.1,pty=bey+bh*0.4,pby=pty+ph,boty=pby-5,boby=boty+boh; const dc=isSelf?dustyPlayerSelfColor:dustyPlayerOtherColor; ctx.save(); ctx.beginPath(); const sy=boby+1; ctx.ellipse(x,sy,w*0.45,h*0.05,0,0,Math.PI*2); ctx.fillStyle=backgroundShadowColor; ctx.fill(); ctx.fillStyle=dc; const lw=w*0.4; ctx.fillRect(x-w*0.45,pty,lw,ph); ctx.fillRect(x+w*0.05,pty,lw,ph); ctx.fillStyle=bootColor; if(!ii){const sd=250,sp=Math.floor(t/sd)%2;if(sp===0){ ctx.fillRect(x-bos-bow/2,boty-2,bow,boh); ctx.fillRect(x+bos-bow/2,boty,bow,boh);}else{ ctx.fillRect(x-bos-bow/2,boty,bow,boh); ctx.fillRect(x+bos-bow/2,boty-2,bow,boh);}}else{ ctx.fillRect(x-bos-bow/2,boty,bow,boh); ctx.fillRect(x+bos-bow/2,boty,bow,boh);} ctx.fillStyle=beltColor; ctx.fillRect(x-w*0.65,bey-bh/2,w*1.3,bh); ctx.fillStyle=simpleChestPlateColor; ctx.fillRect(x-cpw/2,cpty,cpw,cph_local); ctx.fillStyle=chestPlateHighlight; ctx.fillRect(x-cpw/2+5,cpty+5,cpw-10,3); ctx.fillStyle=ironHelmetColor; ctx.fillRect(x-spw/2,sty,spw,sph); ctx.fillStyle=ironHelmetHighlight; ctx.fillRect(x-spw/2+3,sty+2,spw-6,2); ctx.fillStyle=dc; ctx.fillRect(x-spw*0.45,aty,aw,al); ctx.fillRect(x+spw*0.45-aw,aty,aw,al); ctx.fillStyle=ironHelmetColor; ctx.fillRect(x-hw*0.4,ngty,hw*0.8,ngh); ctx.fillStyle=ironHelmetColor; ctx.fillRect(x-hw/2,hty,hw,hh); ctx.fillStyle=ironHelmetHighlight; ctx.fillRect(x-hw/2,hty,hw,3); ctx.fillStyle=ironHelmetShadow; ctx.fillRect(x-hw/2+1,hty+3,hw-2,2); ctx.fillStyle=slitColor; ctx.fillRect(x-slw/2,sly,slw,slh); if(isSelf&&(aimDx!==0||aimDy!==0)){const gunLevel=playerState?.gun??1,glb=12,glbo=(gunLevel-1)*3,gl_local=glb+glbo,gt=5+(gunLevel-1)*0.5,goy=aty+al*0.4,goxo=w*0.1; ctx.save(); ctx.translate(x,goy); const a=Math.atan2(aimDy,aimDx); ctx.rotate(a); ctx.fillStyle=gunColor; ctx.fillRect(goxo,-gt/2,gl_local,gt); ctx.restore();} if(jh){ctx.save(); const ns=15+Math.random()*10; for(let i=0;i<ns;i++){const a=Math.random()*Math.PI*2,r=Math.random()*w*0.8+w*0.2,px=x+Math.cos(a)*r,py=y+Math.sin(a)*r*0.7-h*0.1,ps=Math.random()*3.5+1.5; ctx.fillStyle=sparkColors[Math.floor(Math.random()*sparkColors.length)]; ctx.beginPath(); ctx.arc(px,py,ps/2,0,Math.PI*2); ctx.fill();} ctx.restore();} ctx.restore(); }
    ...
    function drawGame( ctx, appState, stateToRender, localPlayerMuzzleFlashRef, width, height ) {
      ...
      if (!ctx) { console.error("drawGame called without valid context!"); return; } // Guard clause
  
      const now = performance.now();
      canvasWidth = width; canvasHeight = height; // Update dimensions
  
      let shakeApplied = false, shakeOffsetX = 0, shakeOffsetY = 0;
  
      // Apply Shake Calculation
      if (currentShakeMagnitude > 0 && now < shakeEndTime) {
          shakeApplied = true;
          const timeRemaining = shakeEndTime - now;
          // Ensure duration calculation is safe
          const initialDuration = shakeEndTime > 0 ? (shakeEndTime - (shakeEndTime - (currentShakeMagnitude * 1000))) : durationMsFallback; // Use a fallback if calculation fails
          ...
          } else { currentShakeMagnitude = 0; shakeEndTime = 0; shakeApplied = false; } // End shake if magnitude too small
      } else if (currentShakeMagnitude > 0) { currentShakeMagnitude = 0; shakeEndTime = 0; } // Reset if time expired
  
      // --- Render Order ---
      // 1. Background (Always first, fully opaque or transitioning)
      ...
      if (!isBackgroundReady) {
        ...
        if (progress >= 1.0) {
          isTransitioningBackground = false;
          // Ensure state update happens *after* transition visually completes
          currentBackgroundIsNight = offscreenCanvas.dataset.isNight === 'true';
          console.log("[Renderer] Background transition complete.");
        }
      } else {
        ctx.drawImage(offscreenCanvas, 0, 0); // Draw the ready background
      }
  
      // 2. Apply Shake Transform *before* drawing world elements
      if (shakeApplied) { ctx.save(); ctx.translate(shakeOffsetX, shakeOffsetY); }
  
      // 3. Draw Game World Elements (Affected by Shake)
      if (!stateToRender) { // Guard if state is missing
          if (shakeApplied) ctx.restore();
          console.warn("drawGame: stateToRender is missing, skipping world draw.");
          return;
      }
      drawCampfire(ctx, stateToRender.campfire, canvasWidth, canvasHeight);
      // Access global 'snake' carefully
      ...
      // Access global 'active...' carefully
      ...
      // Muzzle Flash (affected by shake)
      ...
      // 4. Restore Shake Transform *before* drawing overlays
      if (shakeApplied) { ctx.restore(); }
  
      // 5. Draw Overlays (Not Affected by Shake)
      ctx.globalAlpha = 1.0; // Ensure alpha reset before overlays
      ...
      if (appState) drawTemperatureTint(ctx, appState.currentTemp, canvasWidth, canvasHeight); // Handles its own alpha

...
const Renderer = (() => {
    ...
    // --- *** NEW: Haze Effect Canvas *** ---
    const hazeCanvas = document.createElement("canvas");
    const hazeCtx = hazeCanvas.getContext("2d", { willReadFrequently: true }); // Might read pixels if using different technique later
  
    // --- Internal Constants ---
    const dayBaseColor = "#8FBC8F";
    const nightBaseColor = "#3E2723";
    const fontFamily = "'Courier New', monospace";
    // ... (keep all other color/style constants) ...
    ...
    const TEMP_HOT_CLIENT = 35.0; // Keep for tint
    const TEMP_SCORCHING_CLIENT = 40.0; // Keep for tint
    ...
    // --- *** NEW: Heat Haze Constants *** ---
    const HEAT_HAZE_START_TEMP = 28.0; // Start shimmer at 28°C
    const HEAT_HAZE_MAX_TEMP = 45.0;   // Full shimmer intensity at 45°C
    const HEAT_HAZE_MAX_OFFSET = 4;    // Max vertical pixel offset for shimmer
    const HEAT_HAZE_SPEED = 0.004;     // How fast the shimmer wave moves
    const HEAT_HAZE_WAVELENGTH = 0.02;  // How wide the shimmer waves are horizontally
    const HEAT_HAZE_LAYERS_MAX = 5;    // Max number of layers drawn for intense heat
    const HEAT_HAZE_BASE_ALPHA = 0.03; // Base transparency per layer
  
    let currentShakeMagnitude = 0;
    let shakeEndTime = 0;
  
    // --- Helper Functions (keep drawRoundedRect, generateBackground, updateGeneratedBackground, etc.) ---
    ...
    function drawGame( ctx, appState, stateToRender, localPlayerMuzzleFlashRef, width, height ) {
      ...
      if (currentShakeMagnitude > 0 && now < shakeEndTime) {
          ...
          const durationMsFallback = 300; // A default duration if calculation fails
          // Ensure initialDuration is positive and reflects intended duration
      ...
      // 1. Draw Background (Handles ready state and transitions)
      ctx.globalAlpha = 1.0;
      if (!isBackgroundReady) {
        ...
        ctx.drawImage(offscreenCanvas, 0, 0); // Draw the current ready background
      }
  
      // --- *** HEAT HAZE DRAWING LOGIC *** ---
      ...
      if (currentTempForEffect !== null && typeof currentTempForEffect !== 'undefined' && currentTempForEffect >= HEAT_HAZE_START_TEMP) {
          ...
          if (hazeIntensity > 0.01) { // Only draw if intensity is noticeable
               // Ensure haze canvas has the right size
              if(hazeCanvas.width !== canvasWidth || hazeCanvas.height !== canvasHeight) {
                  hazeCanvas.width = canvasWidth;
                  hazeCanvas.height = canvasHeight;
              }
              // Copy the *current* main canvas (with background drawn) to haze canvas
              hazeCtx.clearRect(0, 0, canvasWidth, canvasHeight); // Clear previous haze content
              hazeCtx.drawImage(ctx.canvas, 0, 0); // Copy current frame
  
              // Draw hazeCanvas back multiple times with distortion
              const numLayers = 1 + Math.floor(hazeIntensity * (HEAT_HAZE_LAYERS_MAX - 1)); // 1 to MAX layers
              const baseAlpha = HEAT_HAZE_BASE_ALPHA * hazeIntensity;
  
              for (let i = 0; i < numLayers; i++) {
                  // Varying offset based on time, layer, and intensity
                  const timeFactor = now * HEAT_HAZE_SPEED;
                  const layerOffsetFactor = i * 0.8; // How much each layer shifts more
                  const verticalOffset = (Math.sin(timeFactor + layerOffsetFactor) * HEAT_HAZE_MAX_OFFSET * hazeIntensity) - (i * 0.3 * hazeIntensity); // Upward drift
  
                  // Reduce alpha slightly for outer layers
                  const layerAlpha = baseAlpha * (1 - (i / (numLayers * 1.5)));
  
                  ctx.globalAlpha = Math.max(0, Math.min(1, layerAlpha)); // Clamp alpha
                  // Draw the *copied* scene with the vertical offset
                  ctx.drawImage(hazeCanvas,
                      0, 0, canvasWidth, canvasHeight, // Source rect (full canvas)
                      0, verticalOffset, canvasWidth, canvasHeight // Destination rect (with offset)
                  );
              }
               ctx.globalAlpha = 1.0; // Reset alpha after drawing haze
          }
      }
      // --- *** END HEAT HAZE DRAWING LOGIC *** ---
  
  
      // 2. Apply Shake Transform (Applied AFTER background/haze, BEFORE world elements)
      ...
      // Rain / Dust - Draw *before* tint/vignette
      if (appState?.isRaining) {
          ctx.strokeStyle = RAIN_COLOR;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          for (let i = 0; i < RAIN_DROPS; i++) {
              // Basic pseudo-random but deterministic rain position based on time
      ...
      ctx.globalAlpha = 1.0; // Final safety reset
    } // --- End drawGame ---

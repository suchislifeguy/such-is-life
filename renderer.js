...
const Renderer = (() => {
    ...
    const RAIN_COLOR = "rgba(170, 190, 230, 0.6)"; // Semi-transparent blueish grey
    const RAIN_DROPS = 150; // How many drops to draw
    const RAIN_LENGTH = 15;
    const RAIN_SPEED_X = 1; // Slight angle
    const RAIN_SPEED_Y = 12; // How fast they fall vertically
  
    let currentShakeMagnitude = 0;
    let shakeEndTime = 0;
  
    // --- Drawing Helpers (Keep all helper functions as they were) ---
    ...
    function drawGame( ctx, appState, stateToRender, localPlayerMuzzleFlashRef, width, height ) {
      ...
      if (currentShakeMagnitude > 0 && now < shakeEndTime) {
          ...
          const initialDuration = shakeEndTime > now ? (shakeEndTime - (now - durationMsFallback)) : durationMsFallback; // More robust duration estimate
      ...
      // --- *** DRAW WEATHER AND OVERLAYS HERE *** ---
      ctx.globalAlpha = 1.0; // Ensure alpha reset before overlays
  
      // 5. Draw Rain / Dust (Mutually exclusive for now)
      if (appState?.isRaining) {
          ctx.strokeStyle = RAIN_COLOR;
          ctx.lineWidth = 1.5;
          ctx.beginPath(); // Batch rain lines for potential performance gain
          for (let i = 0; i < RAIN_DROPS; i++) {
              const startX = Math.random() * (canvasWidth + 100) - 50; // Start slightly off-screen too
              const startY = Math.random() * canvasHeight;
              const endX = startX + RAIN_SPEED_X;
              const endY = startY + RAIN_SPEED_Y;
              ctx.moveTo(startX, startY);
              ctx.lineTo(endX, endY);
          }
          ctx.stroke(); // Draw all lines at once
      } else if (appState?.isDustStorm) {
          // Dust Storm: Semi-transparent overlay is simpler
          ctx.fillStyle = 'rgba(180, 140, 90, 0.2)';
          ctx.fillRect(0, 0, canvasWidth, canvasHeight);
      }
  
      // 6. Draw Temperature Tint (Already handles alpha)
      if (appState) drawTemperatureTint(ctx, appState.currentTemp, canvasWidth, canvasHeight);
  
      // 7. Draw Vignette (Already handles alpha)

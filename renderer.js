...
const Renderer = (() => {
  ...
  function updateGeneratedBackground(targetIsNight, targetCanvasWidth, targetCanvasHeight) {
    ...
    if (isBackgroundReady) {
      ...
      currentBackgroundIsNight = targetIsNight; // Update state immediately
    } else {
  ...
  // --- FIX: Removed duplicate const declarations ---
  function drawPlayerCharacter( ctx, x, y, w, h, isSelf, playerState, aimDx, aimDy ) {
    const jh=playerState?.hit_flash_this_tick??false,
          ii=(playerState?.input_vector?.dx??0)===0&&(playerState?.input_vector?.dy??0)===0,
          t=performance.now(),
          bo=ii?Math.sin(t/IDLE_BOB_SPEED_DIVISOR)*IDLE_BOB_AMPLITUDE:0;
    const hh=h*0.3, hw=w*0.95, slh=hh*0.15, slw=hw*0.8, ngh=h*0.06,
          spw=w*1.25, sph=h*0.1, chestPlateHeight=h*0.3, cpw=w*0.9, // Used chestPlateHeight here
          aw=w*0.2, al=h*0.4, bh=h*0.05, ph=h*0.35, boh=h*0.1, bow=w*0.32, bos=w*0.4;
    const to=h*0.5, hty=y-to+bo, hby=hty+hh, sly=hty+hh*0.4, ngty=hby-3,
          ngby=ngty+ngh, sty=ngby-2, sby=sty+sph, cpty=sty+sph*0.15,
          aty=sty+sph*0.2, bey=cpty+chestPlateHeight+bh*0.1, // Used chestPlateHeight
          pty=bey+bh*0.4, pby=pty+ph, boty=pby-5, boby=boty+boh;
    ...
    ctx.fillStyle=simpleChestPlateColor; ctx.fillRect(x-cpw/2,cpty,cpw,chestPlateHeight); // Used chestPlateHeight
    ...
    if(isSelf&&(aimDx!==0||aimDy!==0)){
        const gunLevel=playerState?.gun??1, glb=12, glbo=(gunLevel-1)*3;
        const gunLength=glb+glbo; // Used gunLength here
        const gt=5+(gunLevel-1)*0.5,goy=aty+al*0.4,goxo=w*0.1;
        ctx.save(); ctx.translate(x,goy); const a=Math.atan2(aimDy,aimDx); ctx.rotate(a); ctx.fillStyle=gunColor;
        ctx.fillRect(goxo,-gt/2,gunLength,gt); // Used gunLength
        ctx.restore();
    }
    if(jh){ctx.save(); const ns=15+Math.random()*10; for(let i=0;i<ns;i++){const a=Math.random()*Math.PI*2,r=Math.random()*w*0.8+w*0.2,px=x+Math.cos(a)*r,py=y+Math.sin(a)*r*0.7-h*0.1,ps=Math.random()*3.5+1.5; ctx.fillStyle=sparkColors[Math.floor(Math.random()*sparkColors.length)]; ctx.beginPath(); ctx.arc(px,py,ps/2,0,Math.PI*2); ctx.fill();} ctx.restore();}
    ctx.restore();
  }
  ...
  function drawEnemyRect(ctx, x, y, w, h, type, enemyState) {
      const currentW=w, currentH=h; // Use currentW/currentH consistently
      ...
      const ns=performance.now()/1000.0,se=enemyState?.effects?.snake_bite_slow,isb=se&&ns<se.expires_at;
      const hr=currentH*0.18,bheight=currentH*0.5,clb=currentH*0.15,btw=currentW*0.9,bbw=currentW*0.7,cow=currentW*1.1,aw=currentW*0.2,al=currentH*0.4,capH=hr*0.8,capW=hr*2.2,bos=currentW*0.15,bss=currentW*0.3; // Renamed cap height/width vars
      ...
      if(type!=='giant'){ ctx.fillStyle=enemyCapColor; ctx.fillRect(x-capW/2,caty,capW,capH); } // Used capW/capH
      ctx.fillStyle=enemyBootColor; ctx.fillRect(x-bss-bos/2,booy,bos,bos); ctx.fillRect(x+bss-bos/2,booy,bos,bos);
      if(isb){ ctx.fillStyle=snakeBiteTintColor; ctx.fillRect(x-currentW*0.5,y-currentH*0.5+bo,currentW,currentH); }
  ...
  function drawGame( ctx, appState, stateToRender, localPlayerMuzzleFlashRef, width, height ) {
    ...
    if (currentShakeMagnitude > 0 && now < shakeEndTime) {
        ...
        // Simplified duration estimation for decay (might need tuning)
        const estimatedDuration = Math.max(300, shakeEndTime - (performance.now() - timeRemaining)); // Avoid division by zero or negative duration
        let currentMag = currentShakeMagnitude * (timeRemaining / estimatedDuration); // Linear decay
    ...
    if (!isBackgroundReady) {
      ...
      ctx.globalAlpha = 1.0;
      if (progress >= 1.0) { isTransitioningBackground = false; } // Transition visually complete
    } else {
    ...
    // Access global variables carefully
    ...
    if (typeof activeSpeechBubbles !== 'undefined') drawPlayers(ctx, stateToRender.players, appState, localPlayerMuzzleFlashRef);
